            ...
            <fieldset style="margin-top:7px;">
                ...
                if (empty($values_approved) || !is_array($values_approved) || count($values_approved) < 1) {
                    ...
                    <table class="view">
                        ...
                        <?php foreach ($values_approved as $value) { ?>
                            <tr>
                                ...
                                <td><?php echo $this->Js->link('Cancel '.$approval_title, array('controller' => 'LicenseModuleCancelByMfiApprovalDetails', 'action' => 'cancel_approval', $value['LicenseModuleCancelByMfiApprovalDetail']['org_id']), array_merge($pageLoading, array('class' => 'btnlink', 'confirm' => 'Are you sure to cancel approval ?', 'title' => "$approvalType approval cancel")))
                                              .$this->Js->link('Details', array('controller'=>'LicenseModuleCancelByMfiApprovalDetails', 'action'=>'preview', $value['LicenseModuleCancelByMfiApprovalDetail']['org_id'],null),array_merge($pageLoading, array('class'=>'btnlink', 'update'=>'#popup_div')));
                                    ?>
                                </td>
                            </tr>
                        <?php } ?>
                    ...
                    if (count($values_approved) > 1) {
                        ...
                        . $this->Js->link('Update All', array('controller' => 'LicenseModuleCancelByMfiApprovalDetails', 'action' => 'approve_edit_all'), array_merge($pageLoading, array('class' => 'mybtns', 'title' => 'Edit Approval of All Letter')))
                        . '</div>';
                    }
                    ?>
            ...
            <fieldset>
                ...
                if (empty($values_not_approved) || !is_array($values_not_approved) || count($values_not_approved) < 1) {
                        ...
                        <?php foreach ($values_not_approved as $value) { ?>
                            <tr>
                                ...
                                <td style="height:30px; padding:2px; text-align:center;"> 
                                    <?php
                                    echo $this->Js->link($btn_title, array('controller' => 'LicenseModuleCancelByMfiApprovalDetails', 'action' => 'approve', $value['BasicModuleBasicInformation']['id']), array_merge($pageLoading, array('class' => 'btnlink', 'title' => $approval_title.' for Canceling License')))
                                        .$this->Js->link('Details', array('controller'=>'LicenseModuleCancelByMfiApprovalDetails', 'action'=>'preview', $value['BasicModuleBasicInformation']['id'],null),array_merge($pageLoading, array('class'=>'btnlink', 'update'=>'#popup_div')));
                                    ?>
                                </td>
                            </tr>
                        <?php } ?>
                    ...
                    if (count($values_not_approved) > 1) {
                        echo '<div class="btns-div" style="padding:7px; text-align:center;">'
                        . $this->Js->link('Approve All', array('controller' => 'LicenseModuleCancelByMfiApprovalDetails', 'action' => 'approve_all'), array_merge($pageLoading, array('class' => 'mybtns', 'title' => 'Approval for Canceling License')))
                        . '</div>';
                    }
                }
                ?>
