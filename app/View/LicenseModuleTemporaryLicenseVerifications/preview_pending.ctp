<?php                
    if(isset($msg) && !empty($msg)) {
        ...
        if(!empty($allDetails['LicenseModuleTemporaryLicenseVerification'])){
            $status = '';
            if($allDetails['LicenseModuleTemporaryLicenseVerification'][0]['verification_status_id']=='0'){
                $status = 'Not Verified';  
            }
            else if($allDetails['LicenseModuleTemporaryLicenseVerification'][0]['verification_status_id']=='1'){
                $status = 'Verified';  
            } 
            $verification_date = $allDetails['LicenseModuleTemporaryLicenseVerification'][0]['verification_date'];
            $comments = $allDetails['LicenseModuleTemporaryLicenseVerification'][0]['comments'];            
        }
        else{
           $status = 'Pending';
           $verification_date = '';
