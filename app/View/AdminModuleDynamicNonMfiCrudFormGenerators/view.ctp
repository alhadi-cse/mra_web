<fieldset>
    ...
    if (!empty($model_id) && !empty($model_name)) {
        ...
        $model_list = array(98, 99);

//        debug($model_id);
//        debug($values);

        //debug($group_title_in_one_to_many_field_description);
        ?>
        <div>
            <?php
                ...
                if (empty($values) || !is_array($values) || count($values) < 1) {
                    ...
                    <table cellpadding="0" cellspacing="0" border="0">          
                        <tr>
                            ...
                            <td>
                                <?php echo $this->Form->input('search_option', array('label' => false, 'style' => 'width:200px', 'options' => $search_options)); ?>
                            </td>
                            <td class="colons">:</td>
...
//                                echo $this->Js->link("Edit $cat_title", array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'add_multi_cat', $model_id, $cat_id), array_merge($pageLoading, array('class' => 'btnlink')));
//                            } else {
//                                echo $this->Js->link('Edit', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'add_multi', $model_id), array_merge($pageLoading, array('class' => 'btnlink')));
//                            }
//                        } 

                        if (in_array($model_id, $model_list)) {
                            if (!empty($value[$model_name]['loan_category_id'])) {
                                $cat_id = $value[$model_name]['loan_category_id'];
                                echo $this->Js->link("Edit $cat_title", array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'add_multi_cat', $model_id, $cat_id, true), array_merge($pageLoading, array('class' => 'btnlink')));
                            } else {
                                echo $this->Js->link('Edit', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'add_multi', $model_id, true), array_merge($pageLoading, array('class' => 'btnlink')));
                            }
                        } elseif (!empty($value[$model_name][$primary_key_field_name])) {
                            $unique_data_id = $value[$model_name][$primary_key_field_name];
                            echo $this->Js->link('Edit', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'edit', $model_id, $unique_data_id, true), array_merge($pageLoading, array('class' => 'btnlink')));
                            if ($lookup_or_detail_id == 2) {
                                echo $this->Js->link('Details', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'preview', $model_id, $unique_data_id), array_merge($pageLoading, array('update' => '#popup_div', 'class' => 'btnlink')));
                            }
                        }
                ...
                <table style="margin:0 auto; padding:0;" cellspacing="7">
                    <tr>
                        ...
                        <?php if (!empty($user_group_ids) && in_array(5, $user_group_ids)) { ?>
                            <td><?php
                                if (!empty($previous_model_id)) {
                                    echo $this->Js->link('Previous', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => $previous_action_name . '?model_id=' . $previous_model_id), array_merge($pageLoading, array('class' => 'mybtns', 'success' => 'msc.prev();')));
                        ...
                        <td>
                            <?php
                            if (in_array($model_id, $model_list)) {
                                //$btn_add_new = $this->Js->link('Add New', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'add_multi_cat', $model_id), array_merge($pageLoading, array('class' => 'mybtns', 'title' => '')));
                        ...
                        <?php if (!empty($user_group_ids) && in_array(5, $user_group_ids)) { ?>
                            <td><?php echo $this->Js->link('Preview', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => 'individual_preview', $model_id), array_merge($pageLoading, array('class' => 'mybtns', 'update' => '#popup_div'))); ?></td>

                            <td><?php
                                if (!empty($next_model_id)) {
                                    echo $this->Js->link('Next', array('controller' => 'AdminModuleDynamicNonMfiCrudFormGenerators', 'action' => $next_action_name . '?model_id=' . $next_model_id), array_merge($pageLoading, array('class' => 'mybtns', 'success' => 'msc.next();')));
