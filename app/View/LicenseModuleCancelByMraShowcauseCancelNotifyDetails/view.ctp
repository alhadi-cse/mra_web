...
$title = "Show Cause Cancel Notification";
...
<div>
    <fieldset>
        ...
        <div class="form">
            ...
            <?php if(empty($org_id)) { 
                echo $this->Form->create('LicenseModuleCancelByMraShowcauseCancelNotifyDetailSent'); ?>
                <table cellpadding="0" cellspacing="0" border="0">                           
                    <tr>
                        <td style="padding-left:15px; text-align:right;">Search Option</td>
                        <td>
                            <?php 
                                echo $this->Form->input('search_option_sent', 
                        ...
                        <td class="colons">:</td>
                        <td><?php echo $this->Form->input('search_keyword_sent',array('label' => false,'style'=>'width:250px')); ?></td>
                    ...
                    <table class="view">
                        ...
                        <tr>
                            ...
                            <td style='padding:2px; text-align:justify;'>
                                <?php                                        
                                    $showcause_cancel_status_id =  $value['LicenseModuleCancelByMraCancelOrRevokeShowCauseDetail']['showcause_cancel_status_id'];
                                    if($showcause_cancel_status_id=='0'){
                                        echo "<p style='background-color:#037108;color:#fff;'>Treated as <strong>Canceled</strong> and Notification sent</p>";
                                    }
                                    else if($showcause_cancel_status_id=='1'){
                                        echo "<p style='background-color:#037108;color:#fff;'>Revoked Show cause Treating as <strong>Licensed</strong> and Notification sent</p>";
                                    }
                                    else{
                            ...
                            <td style="height:30px; padding:2px; text-align:justify;">
                                <?php 
                                    echo $this->Js->link('Details', array('controller'=>'LicenseModuleCancelByMraShowcauseCancelNotifyDetails', 'action'=>'preview', $value['BasicModuleBasicInformation']['id']), 
            ...
            <fieldset>
                ...
                ?>
                    ...
                    if ($isAdmin && (!empty($pending_values) && is_array($pending_values) && count($pending_values) > 0)) {
                        echo $this->Form->create('LicenseModuleCancelByMraShowcauseCancelNotifyDetailPending');
                    ?>
                    ...
                    <?php if (!empty($pending_values) && is_array($pending_values) && count($pending_values) > 0) { ?>
                        <table class="view">
                            ...
                            <tr>
                                ...
                                <?php
                                    $actions = $this->Js->link('Details', array('controller'=>'LicenseModuleCancelByMraShowcauseCancelNotifyDetails', 'action'=>'preview', $value['BasicModuleBasicInformation']['id']), 
                                                                    array_merge($pageLoading, array('class'=>'btnlink', 'update'=>'#popup_div')));                                                                     

                                    if($thisStateIds[0]==$value['BasicModuleBasicInformation']['licensing_state_id']){
                                        $width = '187px';
                                        $actions = $actions.$this->Js->link('Send Notification', array('controller'=>'LicenseModuleCancelByMraShowcauseCancelNotifyDetails', 'action'=>'send_notification', $value['BasicModuleBasicInformation']['id']), 
