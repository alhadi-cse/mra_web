...
<div id='map_info' class="map_info">
        ...
        <table>
            <tr>
                <td colspan="2" style='vertical-align:top; text-align:left;'>
                    ...
                    <select id="bd_info" style="width:150px; margin-left:5px;">
                        <option value="">--- Select ---</option>
                        <option value="dist">District</option>
                        <option value="upaz">Upazila</option>
                        <!--<option value="div">Division</option>-->
                    </select>
                    <select id="bd_info_dist" style="width:185px; margin-left:15px">
                        <option value="">--- Select ---</option>
                    </select>
                    <select id="bd_info_upaz" style="width:170px; margin-left:5px;display:none">
                        <option value="">--- Select ---</option>
                    </select>
            ...
            <tr>
                <td style='vertical-align:top;'>
                    ...
                    . $this->Form->input('', array('id' => 'org_list', 'type' => 'select', 'class' => 'org_opt', 'style' => 'position:absolute;left:0; width:350px;', 'options' => $org_list, 'value' => $org_id, 'empty' => '--------- All MFIs --------', 'escape' => false, 'div' => false, 'label' => false))
                    . "</div></div></td>"
                    . "<td style='vertical-align:top;'>"
    ...
    <script>
        ...
        var map, map_data_title, bd_center = null, data_info = [], map_data = [], all_data_info = [], all_map_poly = [];
        var lat = 23.777777, log = 90.444444, zooming = 7;
        //23.777777, 90.444444
        ...
        function set_branch_data() {
            ...
            $.ajax({
                ...
                //url: document.location + "ReportModuleReportViewers\/get_loan_info\/" + adminName + "\/" + orgId,
...
//            // || map_data.constructor !== Array
            ...
            for (var dc = 0; dc < map_data.length; dc++) {
                if (!map_data[dc].data_value)
                    continue;
                currVal = parseInt(map_data[dc].data_value, 10);
            ...
            //alert(total_branch_count + ":" + $("#branch_count").html());
            ...
            bd_admin.features.forEach(function (feature) {
                try {
                    ...
                    all_map_poly.push({center: poly_center, div_code: admin_props.div_code, dist_code: admin_props.dist_code, upaz_code: admin_props.upaz_code, map_poly: map_poly, data_value: data_value});
...
//                    curr_admin_name = admin_props[admin_field_name];
////                    curr_admin_code = admin_props[admin_field_code];
//                    admin_field_name + "_" + curr_admin_code;
                    ...
                    //labelOptions.paneId = admin_field_name + "_" + curr_admin_code;
            ...
            if ($("#bd_info_dist").val() || $("#bd_info_upaz").val())
                set_map_admin_boundary();
            else
                set_map_filter();
...
//Moshiur
        ...
        $("#bd_info").on("change", function () {
            ...
            $("#bd_info_dist").empty();
            $("#bd_info_dist").append('<option value="">--- select ---</option>');
            $("#bd_info_upaz").empty();
            $("#bd_info_upaz").append('<option value="">--- select ---</option>');
            for (var fc = 0; fc < bd_all["dist"].features.length; fc++) {
                $("#bd_info_dist").append('<option value="' + bd_all["dist"].features[fc].properties.dist_code + '">' + bd_all["dist"].features[fc].properties.dist_name + '</option>');
            }
...
//            map.setZoom(7);
            ...
            var dstCode = $("#bd_info_dist").val();
            for (var fc = 0; fc < bd_all["upaz"].features.length; fc++) {
                if (bd_all["upaz"].features[fc].properties.dist_code == dstCode) {
                    $("#bd_info_upaz").append('<option value="' + bd_all["upaz"].features[fc].properties.upaz_code + '">' + bd_all["upaz"].features[fc].properties.upaz_name + '</option>');
                }
            }
...
//            set_map_filter();
        ...
        function set_map_admin_boundary() {
            $("#busy-indicator").fadeIn();

            var total_branch_count = 0,
                    distCode = $("#bd_info_dist").val(),
                    upazCode = $("#bd_info_upaz").val();

            if ((!distCode || distCode == '') && (!upazCode || upazCode == '')) {
                for (var pc = 0; pc < all_map_poly.length; pc++) {
                    all_map_poly[pc].map_poly.setMap(map);
                    total_branch_count += parseInt(all_map_poly[pc].data_value, 10);
                }

                map.setCenter(bd_center);
                map.setZoom(7);
            } else {
                //if (all_map_poly[pc].dist_code == distCode && (upazCode && all_map_poly[pc].upaz_code ==upazCode)) {
                for (var pc = 0; pc < all_map_poly.length; pc++) {
                    if (all_map_poly[pc].dist_code == distCode) {
                        if (upazCode) {
                            if (all_map_poly[pc].upaz_code == upazCode) {
                                all_map_poly[pc].map_poly.setMap(map);
                                map.setCenter(all_map_poly[pc].center);
                                map.setZoom(11);
                                total_branch_count += parseInt(all_map_poly[pc].data_value, 10);
                            } else {
                                all_map_poly[pc].map_poly.setMap(null);
        ...
        function set_map_labelX() {
            var lblCls, notCls;

            lblCls = notCls = ".map_label";
            if ($("#map_label_opt").is(':checked')) {
                if ($("#bd_info_dist").val()) {
                    notCls += ":not(.dist_" + $("#bd_info_dist").val() + ")";
                    lblCls += ".dist_" + $("#bd_info_dist").val();
                }

                if ($("#bd_info_upaz").val()) {
                    notCls += ":not(.upaz_" + $("#bd_info_upaz").val() + ")";
                    lblCls += ".upaz_" + $("#bd_info_upaz").val();
                }
        ...
        function set_district_map(dist_id) {
            ...
            var dist_center, mc, total_branch_count = 0;
            if (!dist_id || dist_id == '') {
                for (var pc = 0; pc < all_map_poly.length; pc++) {
                    all_map_poly[pc].map_poly.setMap(map);
                    total_branch_count += parseInt(all_map_poly[pc].data_value, 10);
                ...
                for (var pc = 0; pc < all_map_poly.length; pc++) {
                    if (all_map_poly[pc].dist_code != dist_id) {
                        all_map_poly[pc].map_poly.setMap(null);
                    } else {
                        all_map_poly[pc].map_poly.setMap(map);
                        map.setCenter(all_map_poly[pc].center);
        ...
        function set_upazila_map(upaz_id) {
            ...
            for (var pc = 0; pc < all_map_poly.length; pc++) {
                if (all_map_poly[pc].upaz_code != upaz_id)
                    all_map_poly[pc].map_poly.setMap(null);
                else
                {
                    all_map_poly[pc].map_poly.setMap(map);
