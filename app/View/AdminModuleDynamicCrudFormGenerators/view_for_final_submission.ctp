                    ...
                    foreach ($pending_values as $value) {
                        ...
                        if (!empty($value[$model_name]['branch_id'])) {
                            ...
                            if (!empty($value[$model_name]['loan_category_id'])) {
                                ...
                                $btn_edit = $this->Js->link("Edit $cat_title", array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'add_multi_cat', $model_id, $cat_id, $branch_id), array_merge($pageLoading, array('class' => 'btnlink')));
                            } else {
                                $btn_edit = $this->Js->link('Edit', array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'add_multi', $model_id, $branch_id), array_merge($pageLoading, array('class' => 'btnlink')));
                            }
                        } elseif (!empty($value[$model_name][$primary_key_field_name])) {
                            $unique_data_id = $value[$model_name][$primary_key_field_name];
                            $btn_edit = $this->Js->link('Edit', array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'edit', $model_id, $unique_data_id), array_merge($pageLoading, array('class' => 'btnlink')));
                        }
                        echo $btn_edit;
            ...
            <div class="btns-div">
                <table style="margin:0 auto; padding:0;" cellspacing="7">
                    <tr>
                        ...
                        <td>
                            <?php
                            echo $this->Js->link('Submit All', array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'submit_all', $model_id), array_merge($pageLoading, array('class' => 'mybtns', 'title' => '')));
                            ?>
                        </td>
                        <td>
                            <?php
                            echo $this->Js->link('Show Pending Branches', array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'pending_branch_list', $model_id), array_merge($pageLoading, array('class' => 'mybtns', 'title' => '')));
                            ?>
                        </td>
                        <td></td>
                    </tr>
