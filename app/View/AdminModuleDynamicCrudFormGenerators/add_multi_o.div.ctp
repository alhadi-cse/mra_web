...
<style>
    ...
    .multi_tbl_view {
        ...
        padding: 0;
        color: #222;
        font: normal 12px/1.3 Verdana, Helvetica, Arial, sans-serif;
        border-collapse: collapse;
        border-spacing: 0;
    }
...
if (!empty($field_details_list)) {
    ...
    <fieldset>
        ...
        <ul style="padding:5px 35px; color:#0387B5; font-weight:bold; font-size:13px; list-style:square;">
            <li>Fields with <strong style="color: red;">*</strong> marks are mandatory to fill up</li>
        </ul>
        ...
        if ($field_details_list[0]) {
            ...
            foreach ($basic_field_details_list as $field_id => $field_details) {
                ...
                echo "<tr $tr_style>"
                . "<td style='width:20px; text-align:center;'>$sl_no.</td>"
...
//        debug($basic_field_details_list);
        $dr_header_group = '';
        $dr_header_title = '';
        $dr_header_sub_title = '';
...
//        debug($cat_group_list);
        ...
        //debug($cat_field_details);
        //$cat_level = count($cat_field_list);
        echo $this->Form->input("FormInfo.cat_level", array('type' => 'hidden', 'value' => $cat_level, 'div' => false, 'label' => false));
        ...
        if (!empty($cat_level) && !empty($cat_field_list[0]['options'])) {
            ...
            $cat_options = $cat_field['options'];

            $dr_controls = '';
            foreach ($cat_options as $cat_id => $cat_title) {
                    ...
                    foreach ($sub_cat_options as $sub_cat_id => $sub_cat_title) {
                        ...
                        if ($cat_level > 2 && !empty($cat_field_list[2]['options'])) {
                            ...
                            foreach ($sub_sub_cat_options as $sub_sub_cat_id => $sub_sub_cat_title) {
                                ...
                                //$control_cat_ids = $cat_id * 1000 + $sub_cat_id * 100 + $sub_sub_cat_id;
                                ...
                                $cat_controls = "<strong>$sub_sub_cat_title</strong>"
                                        . $this->Form->input("$control_cat_name.$cat_field_name", array('type' => 'hidden', 'value' => $cat_id, 'div' => false, 'label' => false))
                                        . $this->Form->input("$control_cat_name.$sub_cat_field_name", array('type' => 'hidden', 'value' => $sub_cat_id, 'div' => false, 'label' => false))
                                        . $this->Form->input("$control_cat_name.$sub_sub_cat_field_name", array('type' => 'hidden', 'value' => $sub_sub_cat_id, 'div' => false, 'label' => false));
...
//                                                $field_names = explode(" as ", $field_name);
                                                ...
                                                $dynamic_control = $this->Form->input("", array('id' => "sum_$field_id", 'type' => 'text', 'class' => 'readonly', 'style' => 'width:100px; text-align:right;', 'readonly' => 'readonly', 'div' => false, 'label' => false));
                                                break;
                                            ...
                                            case 'aggregate_label':
                                                ...
                                                $dynamic_control = $this->Form->input("", array('id' => "sum_$className", 'type' => 'text', 'class' => 'readonly', 'style' => 'width:100px; text-align:right;', 'readonly' => 'readonly', 'div' => false, 'label' => false));
                                                break;

                                            case 'text':
                                                switch ($data_type) {
                                                    case 'varchar':
                                                        $dynamic_control = $this->Form->input($control_name, array('id' => $control_id, 'type' => $control_type, 'div' => false, 'label' => false));
                                                        break;
                                                    case 'int':
                                                        $dynamic_control = $this->Form->input($control_name, array('id' => $control_id, 'type' => $control_type, 'class' => "integers $className", 'style' => 'width:100px; text-align:right;', 'div' => false, 'label' => false));
                                                        break;
                                                    case 'double':
                                                        $dynamic_control = $this->Form->input($control_name, array('id' => $control_id, 'type' => $control_type, 'class' => "decimals $className", 'style' => 'width:100px; text-align:right;', 'div' => false, 'label' => false));
                                                        break;
                                                    default:
                                                        break;
                                                }
                                            ...
                                            case 'textarea':
                                                $dynamic_control = $this->Form->input($control_name, array('type' => $control_type, 'id' => $control_id, 'escape' => false, 'div' => false, 'label' => false));
                                                break;

                                            case 'select':
                                                $dynamic_control = $this->Form->input($control_name, array('type' => $control_type, 'options' => $options, 'id' => $control_id, 'empty' => '---Select---', 'div' => false, 'label' => false));
                                                break;

                                            case 'select_or_label':
                                                if (!empty($field_value_from_session)) {
                                                    $value = !empty($options[$field_value_from_session]) ? $options[$field_value_from_session] : '';
                                                    $dynamic_control = $this->Form->input('', array('type' => 'text', 'id' => $control_id, 'value' => $value, 'disabled' => 'disabled', 'div' => false, 'label' => false)) .
                                                            $this->Form->input($control_name, array('type' => 'hidden', 'id' => $control_id, 'value' => $field_value_from_session, 'div' => false, 'label' => false));
                                                } else {
                                                    $dynamic_control = $this->Form->input($control_name, array('type' => 'select', 'options' => $options, 'id' => $control_id, 'empty' => '---Select---', 'div' => false, 'label' => false));
                                                }
                                                break;

                                            case 'dependent_dropdown':
                                                //$dynamic_control = "<div id='control_id_to_be_updated$parent_or_child_control_id'></div>";
                                                $dynamic_control = $this->Form->input($control_name, array('type' => 'select', 'options' => $options, 'id' => 'control_id_to_be_updated' . $parent_or_child_control_id, 'empty' => '---Select---', 'div' => false, 'label' => false));
                                                break;

                                            case 'label':
                                                $dynamic_control = $this->Form->input($control_name, array('type' => 'text', 'id' => $control_id, 'value' => $label, 'disabled' => 'disabled', 'div' => false, 'label' => false));
                                                break;

                                            case 'radio':
                                                $dynamic_control = $this->Form->input($control_name, array('type' => $control_type, 'id' => $control_id, 'options' => $options, 'div' => false, 'label' => false, 'legend' => false));
                                                break;

                                            case 'checkbox':
                                                $options = array();
                                                $dynamic_control = $this->Form->input($control_name, array('type' => $control_type, 'id' => $control_id, 'multiple' => 'checkbox', 'options' => $options, 'escape' => true, 'div' => false, 'label' => false));
                                                break;

                                            case 'date':
                                                $dynamic_control = $this->Form->input($control_name, array('type' => 'hidden', 'id' => $control_id . '_alt', 'div' => false, 'label' => false))
                                                        . "<div><input type='text' id='$control_id' class='date_picker' style='width:100px !important;' /></div>";
                                                break;

                                            case 'current_date':
                                                $dynamic_control = $this->Form->input($control_name, array('type' => 'hidden', 'id' => $control_id, 'value' => $current_date, 'div' => false, 'label' => false));
                                                break;
                                            ...
                                            case 'date_range':
                                                $dynamic_control = $this->Form->input('', array('type' => 'text', 'id' => $control_id, 'value' => $period, 'disabled' => 'disabled', 'div' => false, 'label' => false)) .
                                                        $this->Form->input($control_name, array('type' => 'hidden', 'id' => $control_id, 'value' => $period_id, 'div' => false, 'label' => false));
                                                break;

                                            case 'hidden_date_range':
                                                $dynamic_control = $this->Form->input($control_name, array('type' => 'hidden', 'id' => $control_id, 'value' => $date_value, 'div' => false, 'label' => false));
                                                break;
                                            ...
                                            case 'hidden':
                                                ...
                                                $dynamic_control = $this->Form->input($control_name, array('type' => $control_type, 'id' => $control_id, 'value' => $value, 'div' => false, 'label' => false));
                                                break;
                            ...
                            $dr_sub_controls .= "<td rowspan='$rowspan' class='td_header'><strong>$sub_cat_title</strong></td>";
...
//                            $dr_sub_controls .= '</tr>';
                    ...
                    $dr_cat_controls .= "<td rowspan='$all_sub_rowspan' class='td_header'><strong>$cat_title</strong></td>";
                    $dr_cat_controls .= $dr_sub_controls;
//                    $dr_all_controls .= $dr_cat_controls;
                } else {
                    $dr_cat_controls = '<tr>';
                    $cat_controls = "<strong>$cat_title</strong>" . $this->Form->input("$control_cat_name.$cat_field_name", array('type' => 'hidden', 'value' => $cat_id, 'div' => false, 'label' => false));
        ...
        <div class="btns-div" style="margin-top:5px;"> 
            <table style="margin:0 auto; padding:5px;" cellspacing="7">
                <tr>
                    <td>
                        <?php
                        $close_opt = array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'view?model_id=' . $model_id . '&data_type_id=' . $data_type_id);

                        $is_submit = $this->Session->read('SubmissionStatus.IsSubmit');
                        if (!empty($is_submit) && $is_submit == '1') {
                            $close_opt = array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'view_for_final_submission?model_id=' . $model_id . '&data_type_id=' . $data_type_id . '&is_submit=' . $is_submit);
                        }

                        echo $this->Js->link('Close', $close_opt, array_merge($pageLoading, array('class' => 'mybtns', 'confirm' => 'Are you sure to Close ?')));
                        //echo $this->Js->link('Close', array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => "view?model_id=" . $model_id . "&data_type_id=" . $data_type_id), array_merge($pageLoading, array('class' => 'mybtns', 'confirm' => 'Are you sure to Close ?'))); 
                        ?>
                    </td>
                    <td style="text-align: center;">
                        <?php
                        echo $this->Js->submit($is_data_exist ? 'Update' : 'Save', array_merge($pageLoading, array('class' => 'mybtns', 'confirm' => 'Are you sure to save data ?', 'success' => "msg.init('success', '$title', '$title has been added successfully.');",
...
//echo "<script> $(function () { $all_script }); </script>";
?>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#multi_tbl_view').fixedHeaderTable({footer: true, cloneHeadToFoot: true, altClass: 'odd', autoShow: true});
        $('.fht-thead .fht-table').css({"margin-right": ($('.fht-tbody').hasScrollBar() ? $('.fht-tbody').getScrollBarWidth() : 0) + "px"});
        //$('.fht-tbody table').css("margin-top", -$('.fht-thead .fht-table thead').outerHeight(true) - 1);
