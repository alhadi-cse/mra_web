...
if (!empty($field_details_list)) {
    $noteLoading = array('update' => '#note_content', 'evalScripts' => true, 'class' => 'mybtns',
                        ...
                        foreach ($field_list as $field_id => $field_details) {
                            ...
                            switch ($control_type) {
                                case 'text':
                                    switch ($data_type) {
                                        case 'varchar':
                                            $input_control = $this->Form->input($field_id, array('type' => $control_type, 'label' => false, 'div' => false));
                                            break;
                                        case 'int':
                                            $input_control = $this->Form->input($field_id, array('type' => $control_type, 'class' => 'integers', 'style' => 'width:100px; text-align:right;', 'label' => false, 'div' => false));
                                            break;
                                        case 'double':
                                            $input_control = $this->Form->input($field_id, array('type' => $control_type, 'class' => 'decimals', 'style' => 'width:100px; text-align:right;', 'label' => false, 'div' => false));
                                            break;
                                        default:
                                            break;
                                    }
                                ...
                                case 'textarea':
                                    $input_control = $this->Form->input($field_id, array('type' => $control_type, 'escape' => false, 'label' => false, 'div' => false));
                                    break;
                                ...
                                case 'select_or_label':
                                    if (!empty($field_value_from_session)) {
                                        ...
                                        $input_control = $this->Form->input($field_id, array('type' => 'select', 'options' => $options, 'id' => $control_id, 'empty' => '---Select---', 'label' => false, 'div' => false));
                                    }
                                    break;

                                case 'label':
                                    $input_control = $this->Form->input($field_id, array('type' => 'text', 'value' => $label, 'disabled' => 'disabled', 'label' => false, 'div' => false));
                                    break;

                                case 'radio':
                                    $input_control = $this->Form->input($field_id, array('type' => $control_type, 'options' => $options, 'label' => false, 'div' => false, 'legend' => false));
                                    break;

                                case 'checkbox':
                                    $input_control = $this->Form->input($field_id, array('type' => $control_type, 'multiple' => 'checkbox', 'options' => $options, 'escape' => true, 'label' => false, 'div' => false));
                                    break;

                                case 'date':
                                    $input_control = $this->Form->input($field_id, array('type' => 'hidden', 'id' => $field_name . '_alt', 'label' => false, 'div' => false))
                                ...
                                case 'current_date':
                                    ...
                                    $input_control = $this->Form->input($field_id, array('type' => 'hidden', 'value' => $current_date, 'label' => false, 'div' => false));
                                    break;

                                case 'year_month':
                                    $input_control = $this->Form->year("'" . $note_model_name . "." . $field_id . "'", date('Y') - 15, date('Y'), array('empty' => "--Select Year--", 'style' => 'width:192px;margin:2px 2px 2px 5px;')) . $this->Form->month("'" . $note_model_name . "." . $field_id . "'", array('empty' => "--Select Month--", 'style' => 'width:192px;')) . $this->Form->day("'" . $note_model_name . "." . $field_id . "'", array('empty' => false, 'style' => 'display:none;'));
                                    break;

                                case 'date_range':
                                    $input_control = $this->Form->input('', array('type' => 'text', 'value' => $period, 'disabled' => 'disabled', 'label' => false, 'div' => false));
                                    break;

                                case 'hidden_date_range':
                                    $colon = $field_label = '';
                                    $tr_style = "style='display:none;'";
                                    $input_control = $this->Form->input($field_id, array('type' => 'hidden', 'value' => $date_value, 'label' => false, 'div' => false));
                                    break;
                                ...
                                case 'hidden':
                                    ...
                                    $input_control = $this->Form->input($field_name, array('type' => $control_type, 'value' => $value, 'label' => false, 'div' => false));
                                    break;
