...
<div>
    <fieldset>
        ...
        <?php echo $this->Form->create('LicenseModuleInitialAssessmentAdminApprovalDetailAll'); ?>
        ...
        <div style="width:780px; height:auto; padding:0; overflow-x:auto;">
            <?php 
                if($orgDetails==null || !is_array($orgDetails) || count($orgDetails)<1) {
                    ...
                    echo $this->Js->link('Back', array('controller' => 'LicenseModuleInitialAssessmentAdminApprovalDetails','action' => 'view'),array_merge($pageLoading, array('class'=>'mybtns')));
                }
                else {
                    if (empty($total_marks) || $total_marks < 1) $total_marks = 100;
            ?>

            <table class="view" style="min-width:1200px;">
                <tr>
                    <?php 
                    ...
                    echo "<th style='width:110px;'>Approve All <input type='checkbox' id='chkbApprovalAll'/> </th>";
                    echo "<th style='width:170px;'>Set the Back State <br /><span style='padding:0; color:#fa8713;'>(if not approved)</span></th>";
                    echo "<th style='width:150px;'>Reason <br /><span style='padding:0; color:#fa8713;'>(if not approved)</span></th>";
                    echo "<th style='width:150px;'>Comment</th>";
                ...
                <tr>
                    ...
                    <td>
                        <?php 
                            foreach ($approval_status_options as $value => $text) {
                                echo "<input type='radio' style='margin:2px;' name='data[LicenseModuleInitialAssessmentAdminApprovalDetailAll][$rc][approval_status_id]' ";
                    ...
                    <td>
                        <?php echo $this->Form->input("$rc.back_state_id", array('type' => 'select', 'class' => 'rejOption', 'style'=>'width:170px; padding:1px;', 'options' => $back_states, 'empty' => '---Select---', 'escape' => false, 'div' => false, 'label' => false)); ?>
                    </td>
                    <td>
                        <?php echo $this->Form->input("$rc.reason_if_not_approved",array('type'=>'text', 'style'=>'width:150px; padding:4px;', 'escape'=>false, 'div'=>false, 'label'=>false)); ?>
                    </td>
                    <td>
                        <?php echo $this->Form->input("$rc.comment",array('type'=>'text', 'style'=>'width:150px; padding:4px;', 'escape'=>false, 'div'=>false, 'label'=>false)); ?>
                    </td>
                    <td style="height:30px; padding:2px; text-align:center;"> 
                        <?php 
                            echo $this->Js->link('Assessment Details', array('controller' => 'LicenseModuleInitialAssessmentDetails','action' => 'preview', $orgDetail['BasicModuleBasicInformation']['id']), 
