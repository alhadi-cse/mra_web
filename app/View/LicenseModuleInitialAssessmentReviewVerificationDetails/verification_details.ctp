...
<div>
    <fieldset>
        <legend>Initial Assessment Verification Details</legend>

        <?php
        if (empty($vrificationDetails)) {
            echo 'Did not find any Initial Assessment verification information !';
        } else {
        ?>
        <table cellpadding="7" cellspacing="8" border="0" style="width:90%;">                    
            ...
            <tr>
                ...
                <td class="colons">:</td>
                <td><?php echo $vrificationDetails['LicenseModuleInitialAssessmentReviewVerificationDetail']['verification_date']; ?></td>
            </tr>
            <tr>
                <td>Comment</td>
                <td class="colons">:</td>
                <td><?php echo $vrificationDetails['LicenseModuleInitialAssessmentReviewVerificationDetail']['verification_comment']; ?></td>
            </tr>
            <tr>
                <td>Verified By</td>
                <td class="colons">:</td>
    ...
    <fieldset style="margin-top:7px;">
        <legend>Initial Assessment Verification Approval Details</legend>
        <?php
        if (empty($vrificationApprovalDetails) || !is_array($vrificationApprovalDetails) || count($vrificationApprovalDetails) < 1) {
            echo '<p class="error-message">Did not find any Initial Assessment verification approval information !</p>';
        } else {
        ?>
        <table class="view">
            <tr>
            <?php
                ...
                echo "<th style='width:80px;'>" . $this->Paginator->sort('LicenseModuleInitialAssessmentVerificationApprovalDetail.approval_date', 'Approval Date') . "</th>";
