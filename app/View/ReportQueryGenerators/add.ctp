<div id="frmStatus_add">
    ...
    <fieldset>
        ...
        <?php echo $this->Form->create('ReportQueryGenerator'); ?>
        <div class="form">
        ...
        <div class="btns-div">
            <table style="margin:0 auto; padding:5px;" cellspacing="7">
                <tr>
                    <td>
                        <?php
                        echo $this->Js->link('Close', array('controller' => 'ReportQueryGenerators', 'action' => 'view'), array_merge($pageLoading, array('class' => 'mybtns')));
                        ?>
                    </td>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#add_button').click(function (event) {
            ...
            var field_sorting_order = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.field_sorting_order', array('type' => 'text', 'class' => 'integers', 'label' => false))); ?>;
            field_sorting_order = field_sorting_order.replace(/replace_with_counter/g, counter);

            var field_name = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.field_name', array('type' => 'text', 'label' => false))); ?>;
            field_name = field_name.replace(/replace_with_counter/g, counter);

            var field_description = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.field_description', array('type' => 'text', 'label' => false))); ?>;
            field_description = field_description.replace(/replace_with_counter/g, counter);

            var data_type = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.data_type', array('type' => 'select', 'options' => array('int' => 'Integer', 'varchar' => 'Text', 'double' => 'Double', 'date' => 'Short Date', 'datetime' => 'Date with time'), 'empty' => '---Select---', 'label' => false))); ?>;
            data_type = data_type.replace(/replace_with_counter/g, counter);

            var field_size = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.field_size', array('type' => 'text', 'class' => 'integers', 'label' => false))); ?>;
            field_size = field_size.replace(/replace_with_counter/g, counter);

            var is_primary_key = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.is_primary_key', array('type' => 'radio', 'options' => array('1' => 'Yes', '0' => 'No'), 'label' => false, 'legend' => false))); ?>;
            is_primary_key = is_primary_key.replace(/replace_with_counter/g, counter);

            var is_auto_increment = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.is_auto_increment', array('type' => 'radio', 'options' => array('1' => 'Yes', '0' => 'No'), 'label' => false, 'legend' => false))); ?>;
            is_auto_increment = is_auto_increment.replace(/replace_with_counter/g, counter);

            var is_mandatory = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.is_mandatory', array('type' => 'radio', 'options' => array('1' => 'Yes', '0' => 'No'), 'label' => false, 'legend' => false))); ?>;
            is_mandatory = is_mandatory.replace(/replace_with_counter/g, counter);

            var control_type = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.control_type', array('type' => 'select', 'options' => array('text' => 'Text Box', 'textarea' => 'Text Area', 'select' => 'Dropdown List', 'radio' => 'Radio Button', 'checkbox' => 'Check Box', 'date' => 'Date', 'year_month' => 'Year & Month', 'hidden' => "Hidden/Don't Display"), 'label' => false, 'empty' => '---Select---'))); ?>;
            control_type = control_type.replace(/replace_with_counter/g, counter);

            var select_option_sources = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.select_option_sources', array('type' => 'select', 'options' => $model_options, 'label' => false, 'empty' => '---Select---'))); ?>;
            select_option_sources = select_option_sources.replace(/replace_with_counter/g, counter);

            var css_class_name = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.css_class_name', array('type' => 'select', 'options' => array('integers' => 'Integer', 'decimals' => 'Decimal', 'text' => 'Text', 'date' => 'Date'), 'label' => false, 'empty' => '---Select---'))); ?>;
            css_class_name = css_class_name.replace(/replace_with_counter/g, counter);

            var is_published = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.is_published', array('type' => 'radio', 'options' => array('1' => 'Yes', '0' => 'No'), 'label' => false, 'legend' => false))); ?>;
            is_published = is_published.replace(/replace_with_counter/g, counter);

            var display_in_view_page = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.display_in_view_page', array('type' => 'radio', 'options' => array('1' => 'Yes', '0' => 'No'), 'label' => false, 'legend' => false))); ?>;
            display_in_view_page = display_in_view_page.replace(/replace_with_counter/g, counter);

            var display_in_add_page = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.display_in_add_page', array('type' => 'radio', 'options' => array('1' => 'Yes', '0' => 'No'), 'label' => false, 'legend' => false))); ?>;
            display_in_add_page = display_in_add_page.replace(/replace_with_counter/g, counter);

            var display_in_edit_page = <?php echo json_encode($this->Form->input('ReportQueryGenerator.replace_with_counter.display_in_edit_page', array('type' => 'radio', 'options' => array('1' => 'Yes', '0' => 'No'), 'label' => false, 'legend' => false))); ?>;
