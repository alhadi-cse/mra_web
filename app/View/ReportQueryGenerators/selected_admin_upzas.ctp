...
if (!empty($dist_list) && !empty($upaz_list_all)) {
    ...
    echo $this->Form->create('ReportQueryGeneratorUpazSelect');
    foreach ($dist_list as $dist_id => $dist_name) {
        ...
        $title = "<label for='dist_$dist_id'>$dist_name District </label>" 
                    . "<input type='checkbox' id='dist_$dist_id' class='all-checked' style='margin:0 1px; vertical-align:text-top;'>";
    ...
    $actionLink = array('controller' => 'ReportQueryGenerators', 'action' => 'selected_admin_unions');
    $updateDiv = '#geneQuery';
