<?php                
    if(isset($msg) && !empty($msg)) {
        ...
        if(!empty($allDetails['LicenseModuleLicenseEvaluationVerification'])){
            $status = '';
            if($allDetails['LicenseModuleLicenseEvaluationVerification'][0]['verification_status_id']=='0'){
                $status = 'Not Verified';  
            }
            else if($allDetails['LicenseModuleLicenseEvaluationVerification'][0]['verification_status_id']=='1'){
                $status = 'Verified';  
            } 
            $verification_date = $allDetails['LicenseModuleLicenseEvaluationVerification'][0]['verification_date'];
            $comments = $allDetails['LicenseModuleLicenseEvaluationVerification'][0]['comments'];            
        }
        else{
           $status = 'Pending';
           $verification_date = '';
