<div id="recEvalInfo" title="Detail Information" style="margin:0px; padding:8px; background-color:#fafdff;">
    <div id="tabs">
        <ul>
            <li><a href="#tabs-1">Address</a></li>
            <li><a href="#tabs-2">Comments</a></li>
        </ul>
        <div id="tabs-1">
            <?php echo $this->requestAction(array('controller' => 'BasicModuleBranchInfos', 'action' => 'branch_details', $branch_id), array('return')); ?>            
        </div>
        <div id="tabs-2">
            <?php echo $this->requestAction(array('controller' => 'SupervisionModuleAssignedInspectorApprovalDetails', 'action' => 'approval_details', $supervision_basic_id, $case_id), array('return')); ?>
        </div>
    </div>
</div>

<script>
    $(function () {
        $("#tabs").tabs({active: -1});
        $("#recEvalInfo").dialog({
            modal: true, width: 'auto', height: 'auto', resizable: false, dialogClass: 'my-dialog-box',
            buttons: {
                Close: function () {
                    $(this).dialog("close");
                }
            },
            create: function (evt, ui) {
                $(this).css("minWidth", "820px").css("maxWidth", "1000px");
            }
        });
    });
</script>