...
class LicenseModuleLicenseEvaluationDetailsController extends AppController {
    ...
    public function view($opt = 'all', $mode = null) {
        ...
        if (!empty($org_id)) {
            $condition = array_merge(array('LicenseModuleLicenseEvaluationDetail.org_id' => $org_id), $condition);
        }

        if ($this->request->is('post')) {
            $option = $this->request->data['LicenseModuleLicenseEvaluationDetail']['search_option'];
            $keyword = $this->request->data['LicenseModuleLicenseEvaluationDetail']['search_keyword'];
        ...
        $this->paginate = array('conditions' => $condition2, 'group' => array('LicenseModuleLicenseEvaluationDetail.org_id'), 'limit' => 10, 'order' => array('form_serial_no' => 'asc'));
        $this->LicenseModuleLicenseEvaluationDetail->recursive = 0;
        $this->Paginator->settings = $this->paginate;
        $values_in_progress = $this->Paginator->paginate('LicenseModuleLicenseEvaluationDetail');
        
        $this->paginate = array('conditions' => $condition3, 'group' => array('LicenseModuleLicenseEvaluationDetail.org_id'), 'limit' => 10, 'order' => array('form_serial_no' => 'asc'));
        $this->LicenseModuleLicenseEvaluationDetail->recursive = 0;
        $this->Paginator->settings = $this->paginate;
        $values_approved = $this->Paginator->paginate('LicenseModuleLicenseEvaluationDetail');
    ...
    public function approve_details($org_id = null) {
        ...
        $licApprovalDetails = $this->LicenseModuleLicenseEvaluationDetail->find('first', array('conditions' => array('LicenseModuleLicenseEvaluationDetail.org_id' => $org_id)));
    ...
    public function approve_all() {
        ...
        if ($this->request->is('post')) {
            try {
                $newData = $this->request->data['LicenseModuleLicenseEvaluationDetailAll'];
                if (!empty($newData)) {
                    $this->LicenseModuleLicenseEvaluationDetail->create();
                    if ($this->LicenseModuleLicenseEvaluationDetail->saveAll($newData)) {
        ...
        $approval_status_options = $this->LicenseModuleLicenseEvaluationDetail->LookupLicenseApprovalStatus->find('list', array('fields' => array('LookupLicenseApprovalStatus.id', 'LookupLicenseApprovalStatus.approval_status')));
    ...
    public function approve_edit_all() {
        ...
        if ($this->request->is('post')) {
            try {
                $newData = $this->request->data['LicenseModuleLicenseEvaluationDetailAll'];
                if (!empty($newData)) {
                    $this->LicenseModuleLicenseEvaluationDetail->set($newData);
                    if ($this->LicenseModuleLicenseEvaluationDetail->saveAll($newData)) {
...
//        $orgDetails = $this->Paginator->paginate('BasicModuleBasicInformation');
        ...
        $this->Paginator->settings = $this->paginate;
        $orgDetails = $this->Paginator->paginate('LicenseModuleLicenseEvaluationDetail');
    ...
    public function approve($org_id = null) {
        ...
        if ($this->request->is('post')) {
            try {
                $newData = $this->request->data['LicenseModuleLicenseEvaluationDetail'];

                if (!empty($newData)) {

                    $existingData = $this->LicenseModuleLicenseEvaluationDetail->find('first', array('fields' => array('LicenseModuleLicenseEvaluationDetail.id'), 'conditions' => array('LicenseModuleLicenseEvaluationDetail.org_id' => $newData['org_id'])));

                    if ($existingData) {
                        $this->LicenseModuleLicenseEvaluationDetail->id = $existingData['LicenseModuleLicenseEvaluationDetail']['id'];
                        $done = $this->LicenseModuleLicenseEvaluationDetail->save($newData);
                    } else {
                        $this->LicenseModuleLicenseEvaluationDetail->create();
                        $done = $this->LicenseModuleLicenseEvaluationDetail->save($newData);
                    }
        ...
        $orgDetail = $this->LicenseModuleLicenseEvaluationDetail->BasicModuleBasicInformation->find('first', array('fields' => array('BasicModuleBasicInformation.full_name_of_org', 'BasicModuleBasicInformation.short_name_of_org'), 'conditions' => array('BasicModuleBasicInformation.id' => $org_id), 'recursive' => 0, 'group' => 'BasicModuleBasicInformation.id'));
...
//        $this->set(compact('org_id', 'orgName', 'approval_status_options', 'total_marks', 'pass_min_marks'));
    ...
    public function re_approve($org_id = null) {
        ...
        $approvalDetails = $this->LicenseModuleLicenseEvaluationDetail->find('first', array('conditions' => array('LicenseModuleLicenseEvaluationDetail.org_id' => $org_id)));
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $newData = $this->request->data['LicenseModuleLicenseEvaluationDetail'];
            if (!empty($newData)) {
                $this->LicenseModuleLicenseEvaluationDetail->id = $newData['id'];
                if ($this->LicenseModuleLicenseEvaluationDetail->save($newData)) {
    ...
    public function re_approveP($org_id = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            $newData = $this->request->data['LicenseModuleLicenseEvaluationDetail'];
            if (!empty($newData)) {
                $this->LicenseModuleLicenseEvaluationDetail->id = $approvalDetails['LicenseModuleLicenseEvaluationDetail']['id'];
                if ($this->LicenseModuleLicenseEvaluationDetail->save($newData)) {
                    $this->redirect(array('action' => 'view'));
                }
            }
