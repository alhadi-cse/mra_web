...
class LicenseModuleTemporaryLicenseVerificationsController extends AppController {
    public $helpers = array('Form', 'Html', 'Js', 'Paginator', 'Time');
    public $components = array('Paginator');
    public $paginate = array();
    var $uses = array('BasicModuleBasicInformation','AdminModuleUser','LicenseModuleTemporaryLicenseVerification');
    ...
    public function view() {        
        ...
        $history_data = array();
        $this->LicenseModuleTemporaryLicenseVerification->recursive=1;
        $verification_status_values = $this->LicenseModuleTemporaryLicenseVerification->find('all');
        
        foreach($verification_status_values as $history_value){            
            $history_values['org_id'] = $history_value['LicenseModuleTemporaryLicenseVerification']['org_id'];
            ...
            $history_values['user_id'] = $history_value['LicenseModuleTemporaryLicenseVerification']['user_id'];
            $history_values['user_name'] = $history_value['AdminModuleUserProfile']['full_name_of_user'].'('.$history_value['AdminModuleUser']['user_name'].')';
            $history_values['verification_date'] = $history_value['LicenseModuleTemporaryLicenseVerification']['verification_date'];
            $history_values['verification_status_id'] = $history_value['LicenseModuleTemporaryLicenseVerification']['verification_status_id'];

            if($history_value['LicenseModuleTemporaryLicenseVerification']['verification_status_id']=='1'){
                $history_values['verification_status'] = 'Verified';
            }
            else{
                $history_values['verification_status'] = 'Pending';
            }
            $history_values['comments'] = $history_value['LicenseModuleTemporaryLicenseVerification']['comments'];
        ...
        if ($this->request->is('post')){
            $completed_option = $this->request->data['LicenseModuleTemporaryLicenseVerificationCompleted']['completed_search_option'];  
            $completed_keyword = $this->request->data['LicenseModuleTemporaryLicenseVerificationCompleted']['completed_search_keyword'];
            $completed_condition = array('LicenseModuleTemporaryLicenseVerification.verification_status_id' => '0',"$completed_option LIKE '%$completed_keyword%'"); 

            $this->paginate = array(
            'order' => array('LicenseModuleTemporaryLicenseVerification.id' => 'ASC'),
            'limit' => 8,
            'conditions' => $completed_condition);            
            $this->Paginator->settings = $this->paginate;
            $completed_values = $this->Paginator->paginate('LicenseModuleTemporaryLicenseVerification');            
        }
...
User.
        ...
        if(!in_array(1,$user_group_id )){
            ...
            $condition = array('LicenseModuleTemporaryLicenseVerification.user_id'=>$user_id,'LicenseModuleTemporaryLicenseVerification.org_id'=>$org_id);
            $this->LicenseModuleTemporaryLicenseVerification->recursive = -1;
            $cancel_request_verification_infos = $this->LicenseModuleTemporaryLicenseVerification->find('first',array('conditions'=>$condition));
        ...
        if ($this->request->is('post')){
            if(!empty($cancel_request_verification_infos)){
                ...
                $verification_status_id = $this->request->data['LicenseModuleTemporaryLicenseVerification']['verification_status_id'];
                $comments = $this->request->data['LicenseModuleTemporaryLicenseVerification']['comments'];
                ...
                if($verification_status_id=='0'){//delete all
                    $this->LicenseModuleTemporaryLicenseVerification->deleteAll(array('LicenseModuleTemporaryLicenseVerification.org_id' => $org_id), false);
                    ...
                    if($is_exists){
                        ...
                        $this->LicenseModuleTemporaryLicenseVerification->create();
                        $this->LicenseModuleTemporaryLicenseVerification->save($data_to_save_in_cancel_request_verification);                    
                    }
                    ...
                    $verification_count = $this->LicenseModuleTemporaryLicenseVerification->find('count', array('conditions' => array('LicenseModuleTemporaryLicenseVerification.org_id'=>$org_id)));
                    $verification_infos = $this->LicenseModuleTemporaryLicenseVerification->find('all',array('conditions'=>array('LicenseModuleTemporaryLicenseVerification.org_id'=>$org_id)));
                    $is_approved_by_all = false;
                    foreach($verification_infos as $info){
                        $status_id = $info['LicenseModuleTemporaryLicenseVerification']['verification_status_id'];
    ...
    public function preview_history($user_id = null, $org_id = null){        
        $history_data = array();
        $this->LicenseModuleTemporaryLicenseVerification->recursive=2;
        $allDetails = $this->LicenseModuleTemporaryLicenseVerification->find('first', array('conditions'=>array('LicenseModuleTemporaryLicenseVerification.org_id' => $org_id, 'LicenseModuleTemporaryLicenseVerification.user_id' => $user_id)));        
        $this->set(compact('allDetails'));
    }
