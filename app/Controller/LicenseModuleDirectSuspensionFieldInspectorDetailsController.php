...
class LicenseModuleDirectSuspensionFieldInspectorDetailsController extends AppController {
    ...
    public function view() {        
        ...
        if (!empty($orgIds_assign)) {
            $allDistIds = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->find('list', array('fields' => 'district_id', 'conditions' => array('org_id' => $orgIds_assign), 'group' => array('org_id'))); //, 'conditions' => array('LicenseModuleDirectSuspensionFieldInspectorDetail.district_id' => 'LookupAdminBoundaryDistrict.id')));

            if (!empty($allDistIds)) {

                $allDist = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => array('LookupAdminBoundaryDistrict.id', 'LookupAdminBoundaryDistrict.district_name'), 'conditions' => array('LookupAdminBoundaryDistrict.id' => $allDistIds))); //, 'conditions' => array('LicenseModuleDirectSuspensionFieldInspectorDetail.district_id' => 'LookupAdminBoundaryDistrict.id')));
                foreach ($allDist as $dist_id => $dist_name) {

                    $allOrgIds = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->find('list', array('fields' => 'org_id', 'conditions' => array('district_id' => $dist_id), 'group' => 'org_id'));
                    if (empty($allOrgIds))
                        continue;
                    ...
                    foreach ($allOrgIds as $orgId) {
                        $allInspector = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->find('list', array('fields' => 'name_with_designation_and_dept', 'recursive' => 0, 'conditions' => array('LicenseModuleDirectSuspensionFieldInspectorDetail.org_id' => $orgId)));
                        $condition_assign = array('BasicModuleBasicInformation.id' => $orgId,
                            
                            'BasicModuleBasicInformation.licensing_state_id' => $thisStateIds[2]);
                        
                        //$orgDetails = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->BasicModuleBasicInformation->find('first', array('fields' => array('form_serial_no', 'short_name_of_org', 'full_name_of_org'), 'conditions' => $condition_assign));
                        
                        $fields = array('LicenseModuleDirectSuspensionFieldInspectorDetail.inspection_date', 'BasicModuleBasicInformation.form_serial_no', 'BasicModuleBasicInformation.short_name_of_org', 'BasicModuleBasicInformation.full_name_of_org');
                        $orgDetails = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->find('first', array('fields' => $fields, 'recursive' => 0, 'conditions' => $condition_assign, 'group' => 'LicenseModuleDirectSuspensionFieldInspectorDetail.org_id'));

                        if (empty($orgDetails))
                            continue;
                        //$orgDetails = $this->BasicModuleBasicInformation->find('first', array('fields' => array('form_serial_no', 'short_name_of_org', 'full_name_of_org'), 'conditions' => array('BasicModuleBasicInformation.id' => $orgId)));
                        ...
                        $ins_date = date("d-m-Y", strtotime($orgDetails['LicenseModuleDirectSuspensionFieldInspectorDetail']['inspection_date']));
                ...
                if (!empty($allRows)) {
                    $headerRows = '<tr><th style="width:100px;">District</th>'
                                    . '<th style="width:70px;">Form Serial No.</th>'
...
//            
                ...
                $fields = array('BasicModuleBasicInformation.id', 'BasicModuleBasicInformation.form_serial_no', 'BasicModuleBasicInformation.short_name_of_org', 'BasicModuleBasicInformation.full_name_of_org', 'LookupAdminBoundaryDistrict.district_name');
                //$condition = array('BasicModuleBranchInfo.office_type_id' => $office_type_id);
...
//        }
    ...
    public function assign() {
        ...
        if (!empty($dist_ids)) {
            ...
            $dist_list = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => $fields, 'conditions' => $condition1));
...
//        }
        ...
        if ($this->request->is('post')) {
            ...
            $posted_data = $this->request->data['LicenseModuleDirectSuspensionFieldInspectorDetail'];
            ...
            if (!empty($posted_data)) {
                ...
                if (!empty($new_data_to_save)) {
                    $this->LicenseModuleDirectSuspensionFieldInspectorDetail->deleteAll(array('LicenseModuleDirectSuspensionFieldInspectorDetail.org_id' => $all_org_ids), false);
                    
                    $this->LicenseModuleDirectSuspensionFieldInspectorDetail->create();
                    $done = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->saveAll($new_data_to_save);
                    debug($done);
                    if ($done) {
                        $this->loadModel('BasicModuleBasicInformation');
...
//                $passed_org_ids = $this->LicenseModuleInitialAssessmentDetail->find('list', array('fields' => array('LicenseModuleInitialAssessmentDetail.org_id'), 'recursive' => 0, 'conditions' => array_merge($condition, array('LicenseModuleInitialAssessmentMark.total_assessment_marks >= ' => $pass_min_marks)), 'group' => 'LicenseModuleInitialAssessmentDetail.org_id'));
            ...
            $selected_org_list = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->find('list', array('fields' => array('org_id'), 'conditions' => array('district_id' => $district_id), 'group' => array('org_id')));
        }
...
//            $inspector_list = $this->AdminModuleUserProfile->find('list', array('fields' => $fields, 'conditions' => $conditions));
            ...
            $selected_inspector_list = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->find('list', array('fields' => array('LicenseModuleDirectSuspensionFieldInspectorDetail.inspector_user_id'), 'conditions' => array('LicenseModuleDirectSuspensionFieldInspectorDetail.district_id' => $district_id)));
        }
    ...
    function grid_view_select() {
        $district_id = $this->request->data['LicenseModuleDirectSuspensionFieldInspectorDetail']['district_id'];
        $condition = array('LicenseModuleDirectSuspensionFieldInspectorDetail.district_id' => $district_id);
        $this->LicenseModuleDirectSuspensionFieldInspectorDetail->recursive = 0;
        $this->Paginator->settings = $this->paginate;
        $values = $this->Paginator->paginate('LicenseModuleDirectSuspensionFieldInspectorDetail');
        $this->paginate = array(
            'order' => array('LicenseModuleDirectSuspensionFieldInspectorDetail.id' => 'ASC'),
            'limit' => 10,
            'conditions' => $condition);
        $field_inspection_values = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->find('all', array('conditions' => array('LicenseModuleDirectSuspensionFieldInspectorDetail.district_id' => $district_id)));

        $this->loadModel('BasicModuleBasicInformation');
        $org_list_values = array();
        $inspector_list_values = array();
        foreach ($field_inspection_values as $value) {
            $org_id = $value['LicenseModuleDirectSuspensionFieldInspectorDetail']['org_id'];
    ...
    public function re_assign($id = null, $inspector_user_id = null) {
        ...
        //$dist_list = $this->LicenseModuleDirectSuspensionFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => array('LookupAdminBoundaryDistrict.id', 'LookupAdminBoundaryDistrict.district_name')));
        ...
        if ($this->request->is('post')) {
            try {
                ...
                $this->LicenseModuleDirectSuspensionFieldInspectorDetail->deleteAll(array('LicenseModuleDirectSuspensionFieldInspectorDetail.district_id' => $district_id), false);
                ...
                foreach ($org_ids as $org_id) {
                    foreach ($inspector_user_ids as $inspector_user_id) {
                        if ($inspector_user_id != "") {
                            ...
                            $this->LicenseModuleDirectSuspensionFieldInspectorDetail->save($data_to_save);
                            $flag = true;
                        } else {
                            $flag = false;
                            break;
