...
class AdminModuleDynamicCrudFormGeneratorsController extends AppController {

    public $helpers = array('Html', 'Form', 'Js', 'Paginator', 'Time');
    public $components = array('Paginator');
    public $org_name = "";
    public $branch_with_code_address = "CONCAT_WS(', ', CONCAT_WS('', BasicModuleBranchInfo.branch_name, CASE WHEN BasicModuleBranchInfo.branch_name != '' AND BasicModuleBranchInfo.branch_code != '' THEN CONCAT_WS('', ' (', BasicModuleBranchInfo.branch_code, ')') ELSE BasicModuleBranchInfo.branch_code END), road_name_or_village, LookupAdminBoundaryUpazila.upazila_name, LookupAdminBoundaryDistrict.district_name)";

    //public $branch_with_code_address = "CONCAT_WS(', ', CONCAT_WS('', BasicModuleBranchInfo.branch_name, CASE WHEN BasicModuleBranchInfo.branch_name != '' AND BasicModuleBranchInfo.branch_code != '' THEN CONCAT_WS('', ' (', BasicModuleBranchInfo.branch_code, ')') ELSE BasicModuleBranchInfo.branch_code END), road_name_or_village, mohalla_or_post_office, LookupAdminBoundaryUpazila.upazila_name, LookupAdminBoundaryDistrict.district_name)";

    public function add_multi($model_id, $branch_id = null) {
        ...
        if (empty($branch_id))
            $branch_id = $this->Session->read('Org.BranchId');
//        else
//            $this->Session->write('Org.BranchId', $branch_id);
//        $office_type_id = 3;
//        $branch_type_id = 3;
...
//                    $this->$model_name->save($reqData);
                ...
                if ($all_is_ok) {
                    try {
                        if (!empty($period_id) && !empty($org_id) && !empty($basic_data['branch_id'])) {
                            $conditions = array("$model_name.period_id" => $period_id, "$model_name.org_id" => $org_id, "$model_name.branch_id" => $basic_data['branch_id']);
                    ...
                    if (count($forSaveDatas) > 0) {
                        ...
                        if ($isDone) {
                            ...
                            if (!empty($is_submit) && $is_submit == '1') {
                                $this->redirect(array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'view_for_final_submission?model_id=' . $model_id . '&data_type_id=' . $data_type_id . '&is_submit=' . $is_submit));
                            } else {
                                $this->redirect(array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'view?model_id=' . $model_id . '&data_type_id=' . $data_type_id));
                            }
                            return;
                        }
                    } else {
        ...
        if (!empty($period_id)) {
            ...
            $redirect_url = array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'add', $model_id);
        ...
        if (!empty($all_fields_details)) {
            ...
            if ($this->$model_name->hasField('branch_id'))
                $sent_branch_ids = $this->$model_name->find('list', array('fields' => array("$model_name.branch_id"), 'conditions' => array("$model_name.org_id" => $org_id, "$model_name.period_id" => $period_id), 'group' => array("$model_name.branch_id"), 'recursive' => -1));
            else
                $sent_branch_ids = null;
            ...
            foreach ($all_fields_details as $field_details) {
                ...
                } elseif ($field_name == 'branch_id') {
                    $field_value_from_session = $branch_id;
                } else {
                ...
                switch ($control_type) {
                    ...
                    case "dependent_dropdown": {
                            ...
                            if ($field_name == 'branch_id' && !empty($dropdown_value_field)) {
                                $order_by = $fields[1];
                                $value_exist_conditions["$select_option_model.office_type_id"] = $branch_office_type_id;
                                if (!empty($branch_id))
                                    $value_exist_conditions["$select_option_model.$dropdown_value_field"] = $branch_id;
                                elseif (!empty($sent_branch_ids)) {
                                    $value_exist_conditions["NOT"] = array("$select_option_model.$dropdown_value_field" => $sent_branch_ids);
                                }
                            }
...
//            $cat_group_id = $model_id . '01';
            ...
            if (!$this->request->is('post') && !empty($cat_fields) && !empty($branch_id)) {
                $conditions = array();
            ...
            $this->set(compact('is_data_exist'));
        } else {
            $msg = array(
                'type' => 'warning',
                'title' => 'Warning... . . !',
    ...
    public function add_multi_cat($model_id = null, $cat_id = null, $branch_id = null) {
...
//                    $this->$model_name->save($reqData);
        ...
        //$this->BasicModuleBranchInfo->virtualFields = array('branch_with_address' => "CONCAT_WS(', ', CONCAT_WS('', BasicModuleBranchInfo.branch_name, CASE WHEN BasicModuleBranchInfo.branch_name != '' AND BasicModuleBranchInfo.branch_code != '' THEN CONCAT_WS('', ' (', BasicModuleBranchInfo.branch_code, ')') ELSE BasicModuleBranchInfo.branch_code END), LookupAdminBoundaryUpazila.upazila_name, LookupAdminBoundaryDistrict.district_name)");
//        $sent_branch_ids = $this->$model_name->find('list', array(
//            'fields' => array("$model_name.branch_id"),
//            'conditions' => array("$model_name.org_id" => $org_id, "$model_name.period_id" => $period_id, "$model_name.period_id" => $period_id, "$model_name.$cat_fields[0]" => $cat_id),
//            'group' => array("$model_name.branch_id"),
//            'recursive' => -1));
        //$cat_group_id = $model_id . '01';
//        $cat_fields = array();
...
//            $cat_fields = Hash::extract($all_fields_details, "{n}.LookupModelFieldDefinition[field_group_id=$cat_group_id].field_name");
            ...
            if ($this->$model_name->hasField('branch_id'))
                $sent_branch_ids = $this->$model_name->find('list', array('fields' => array("$model_name.branch_id"), 'conditions' => $conditions, 'group' => array("$model_name.branch_id"), 'recursive' => -1));
            else
                $sent_branch_ids = null;
...
//                                $options = Hash::combine($options_list, "{n}.$select_option_model.$dropdown_value_field", "{n}.$select_option_model.$dropdown_display_field", "{n}.$select_option_model.$dependent_dropdown_condition_field");
            ...
            $is_data_exist = false;
            if (!$this->request->is('post') && !empty($cat_fields) && !empty($branch_id) && !empty($cat_fields[0]) && !empty($cat_id)) {
                $conditions = array();
                $conditions['period_id'] = $period_id;
                $conditions['org_id'] = $org_id;
                $conditions['branch_id'] = $branch_id;
        ...
        if (!empty($model_details)) {
            ...
            foreach ($field_values as $field_value) {
                ...
                if ($is_available_in_view_condition == '1') {
                    ...
                    if ($condition_field_name == 'branch_id' && !empty($branch_id)) {
                        $value_exist_conditions["$model_name.$condition_field_name"] = $branch_id;
                        $submitted_condition["$model_name.$condition_field_name"] = $branch_id;
                        //$condition["$model_name.$condition_field_name"] = $branch_id;
                    }
...
//                    else if ($this->$model_name->hasField('branch_id'))
//                        $paginate_opts['group'] = "$model_name.branch_id";
...
//        $this->BasicModuleBranchInfo->virtualFields['branch_with_address'] = $this->branch_with_code_address;
        ...
        $org_values = $this->BasicModuleBasicInformation->find('first', array('conditions' => array('BasicModuleBasicInformation.id' => $org_id), 'recursive' => -1));
        if (!empty($org_values['BasicModuleBasicInformation'])) {
            $name_of_org = $org_values['BasicModuleBasicInformation']['name_of_org'];
        ...
        if (!empty($model_details)) {
            ...
            //$this->$model_name->virtualFields['branch_with_address'] = $this->BasicModuleBranchInfo->virtualFields['branch_with_address'];
...
//			'field_description' => 'Branch Name',
//			'associated_field_name_to_show' => 'branch_name',
//			'associated_model_name' => 'BasicModuleBranchInfo'
//		)
...
//                                debug($this->$model_name->$bind_type['BasicModuleBasicInformation']);
...
//            $this->BasicModuleBranchInfo->virtualFields['branch_with_address'] = $this->branch_with_code_address;
    ...
    public function pending_branch_list($opt = null) {
        $this->loadModel('LookupModelDefinition');
        $this->loadModel('LookupModelFieldDefinition');
        $this->loadModel('BasicModuleBranchInfo');
        ...
        if (!empty($model_details)) {
            ...
            if (!empty($period_values)) {
                ...
                $data_period = $period_values['AdminModulePeriodList']['period'];
                ...
                $sent_branch_ids = $this->$model_name->find('list', array(
                    ...
                    'group' => array("$model_name.period_id", "$model_name.org_id", "$model_name.branch_id"),
                    'recursive' => -1));

                $org_name = $this->$model_name->BasicModuleBasicInformation->field('name_of_org', array('BasicModuleBasicInformation.id' => $org_id));
//                $this->$model_name->virtualFields['name_of_org'] = $this->$model_name->BasicModuleBasicInformation->virtualFields['name_of_org'];
//                $this->$model_name->recursive = 1;
//                $org_name = $this->$model_name->field('name_of_org', array('conditions' => array("$model_name.org_id" => $org_id, "$model_name.period_id" => $period_id)));

                if (empty($org_name)) {
                    $msg = array(
                        'type' => 'warning',
                        'title' => 'Warning... . . !',
                        'msg' => 'No Information Exists!'
                    );
                    $this->set(compact('msg'));
                    return;
                }

//                $unbind_models = array('belongsTo' => array('LookupBasicOfficeType', 'LookupAdminBoundaryDistrict', 'LookupAdminBoundaryUpazila', 'LookupAdminBoundaryUnion', 'LookupAdminBoundaryMauza', 'BasicModuleBasicInformation'),
//                    );
//                $unbind_models = array('belongsTo' => array('BasicModuleBasicInformation', 'LookupBasicOfficeType', 'LookupAdminBoundaryUnion', 'LookupAdminBoundaryMauza'),
//                    ));
                //$this->BasicModuleBranchInfo->virtualFields = array('branch_with_address' => "CONCAT_WS(', ', CONCAT_WS('', BasicModuleBranchInfo.branch_name, CASE WHEN BasicModuleBranchInfo.branch_name != '' AND BasicModuleBranchInfo.branch_code != '' THEN CONCAT_WS('', ' (', BasicModuleBranchInfo.branch_code, ')') ELSE BasicModuleBranchInfo.branch_code END), LookupAdminBoundaryUpazila.upazila_name, LookupAdminBoundaryDistrict.district_name)");
                //$this->BasicModuleBranchInfo->virtualFields = array('branch_with_address' => "CONCAT_WS(', ', BasicModuleBranchInfo.branch_name, LookupAdminBoundaryUpazila.upazila_name, LookupAdminBoundaryDistrict.district_name)");
                ...
                unset($this->BasicModuleBranchInfo->belongsTo['BasicModuleBasicInformation']);
                unset($this->BasicModuleBranchInfo->belongsTo['LookupBasicOfficeType']);
                unset($this->BasicModuleBranchInfo->belongsTo['LookupAdminBoundaryUnion']);
                unset($this->BasicModuleBranchInfo->belongsTo['LookupAdminBoundaryMauza']);

                $fields = array('BasicModuleBranchInfo.id', 'BasicModuleBranchInfo.branch_with_address');

                $value_exist_conditions = array('BasicModuleBranchInfo.org_id' => $org_id, 'BasicModuleBranchInfo.office_type_id' => $branch_office_type_id, array('NOT' => array('BasicModuleBranchInfo.id' => $sent_branch_ids)));
                $pending_branches_names = $this->BasicModuleBranchInfo->find('list', array('fields' => $fields, 'conditions' => $value_exist_conditions, 'recursive' => 0, 'order' => !empty($fields[1]) ? $fields[1] : null));

                $sent_conditions = array('BasicModuleBranchInfo.org_id' => $org_id, 'BasicModuleBranchInfo.office_type_id' => $branch_office_type_id, 'BasicModuleBranchInfo.id' => $sent_branch_ids);
                $sent_branches_names = $this->BasicModuleBranchInfo->find('list', array('fields' => $fields, 'conditions' => $sent_conditions, 'recursive' => 0, 'order' => !empty($fields[1]) ? $fields[1] : null));
            } else {
                $redirect_url = array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'add', $model_id);
                $this->Session->write('Current.RedirectUrl', $redirect_url);
            ...
            if ($this->request->is('post')) {
                if (!empty($this->request->data['BasicModuleBranchInfo_Pending'])) {
                    $pending_option = $this->request->data['BasicModuleBranchInfo_Pending']['search_option'];
                    $pending_keyword = $this->request->data['BasicModuleBranchInfo_Pending']['search_keyword'];
                    $pending_search_condition = array("$pending_option LIKE '%$pending_keyword%'");

                    if (!empty($pending_search_condition)) {
                        $pending_search_condition['BasicModuleBranchInfo.org_id'] = $org_id;
                        $pending_search_condition['BasicModuleBranchInfo.office_type_id'] = $branch_office_type_id;
                        $pending_search_condition['NOT'] = array('BasicModuleBranchInfo.id' => $sent_branch_ids);

                        //$pending_search_condition = array_merge($pending_search_condition, array('BasicModuleBranchInfo.org_id' => $org_id, 'BasicModuleBranchInfo.office_type_id' => $branch_office_type_id, array('NOT' => array('BasicModuleBranchInfo.id' => $sent_branch_ids))));
                        $pending_branches_names = $this->BasicModuleBranchInfo->find('list', array('fields' => $fields, 'conditions' => $pending_search_condition, 'recursive' => 0, 'order' => !empty($fields[1]) ? $fields[1] : null));

//                        $this->paginate = array(
//                            'fields' => $fields,
//                            'conditions' => array_merge($pending_search_condition, array('BasicModuleBranchInfo.org_id' => $org_id, 'BasicModuleBranchInfo.office_type_id' => $branch_office_type_id, array('NOT' => array('BasicModuleBranchInfo.id' => $sent_branch_ids)))),
//                            'recursive' => 0,
//                            'maxLimit' => 5000,
//                            'limit' => 5000
//                        );
//                        $this->Paginator->settings = $this->paginate;
//                        $pending_branches_names = $this->Paginator->paginate('BasicModuleBranchInfo');
                    }
                }

                if (!empty($this->request->data['BasicModuleBranchInfo_Sent'])) {
                    $sent_option = $this->request->data['BasicModuleBranchInfo_Sent']['search_option'];
                    $sent_keyword = $this->request->data['BasicModuleBranchInfo_Sent']['search_keyword'];
                    $sent_search_condition = array("$sent_option LIKE '%$sent_keyword%'");

                    if (!empty($sent_search_condition)) {
                        $sent_search_condition['BasicModuleBranchInfo.org_id'] = $org_id;
                        $sent_search_condition['BasicModuleBranchInfo.office_type_id'] = $branch_office_type_id;
                        $sent_search_condition['BasicModuleBranchInfo'] = $sent_branch_ids;

                        //$sent_search_condition = array_merge($sent_search_condition, array('BasicModuleBranchInfo.org_id' => $org_id, 'BasicModuleBranchInfo.office_type_id' => $branch_office_type_id, 'BasicModuleBranchInfo.id' => $sent_branch_ids));
                        $sent_branches_names = $this->BasicModuleBranchInfo->find('list', array('fields' => $fields, 'conditions' => $sent_search_condition, 'recursive' => 0, 'order' => !empty($fields[1]) ? $fields[1] : null));

//                        $this->paginate = array(
//                            'fields' => $fields,
//                            'conditions' => array_merge($sent_search_condition, array('BasicModuleBranchInfo.org_id' => $org_id, 'BasicModuleBranchInfo.office_type_id' => $branch_office_type_id, 'BasicModuleBranchInfo.id' => $sent_branch_ids)),
//                            'recursive' => 0,
//                            'maxLimit' => 5000,
//                            'limit' => 5000
//                        );
//                        $this->Paginator->settings = $this->paginate;
//                        $sent_branches_names = $this->Paginator->paginate('BasicModuleBranchInfo');
                    }
                }
            }
        }

        $this->set(compact('model_name', 'title', 'org_name', 'data_period', 'pending_branches_names', 'sent_branches_names'));
    }
...
//            $this->loadModel('BasicModuleBranchInfo');
//            $this->BasicModuleBranchInfo->virtualFields = $this->branch_with_code_address; //array('branch_with_address' => "CONCAT_WS(', ', BasicModuleBranchInfo.branch_name, LookupAdminBoundaryUpazila.upazila_name, LookupAdminBoundaryDistrict.district_name)");
        }
    ...
    public function add($model_id = null) {
        ...
        $org_id = $this->Session->read('Org.Id');
        $branch_id = $this->Session->read('Org.BranchId');
        $branch_office_type_id = $this->Session->read('Org.BranchTypeId');
        $data_type_id = $this->Session->read('AdminModulePeriodDataType.DataTypeId');
...
//        debug($org_id);
        ...
        $all_fields_details = $this->LookupModelFieldDefinition->find('all', array('conditions' => array('model_id' => $model_id, 'display_in_add_page' => 1), 'recursive' => -1, 'order' => array('field_sorting_order' => 'asc', 'field_id' => 'asc')));
            ...
            foreach ($all_fields_details as $field_details) {
                ...
                switch ($control_type) {
                    ...
                    case "calculated_label":
                        $new_field_details['field_name'] = $this->get_html_id($field_name);
                        break;
...
//                                $options = Hash::combine($options_list, "{n}.$select_option_model.$dropdown_value_field", "{n}.$select_option_model.$dropdown_display_field", "{n}.$select_option_model.$dependent_dropdown_condition_field");
    ...
    public function edit($model_id = null, $unique_data_id = null, $submission_status_id = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            if ($this->$model_name->save($this->request->data)) {
                ...
                if (!empty($data_type_id)) {
                    ...
                    $this->redirect(array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'view?model_id=' . $model_id));
                }
                return;
            }
        }
        ...
        $all_fields_details = $this->LookupModelFieldDefinition->find('all', array('conditions' => array('model_id' => $model_id, 'display_in_edit_page' => 1), 'recursive' => -1, 'order' => array('field_sorting_order' => 'asc', 'field_id' => 'asc')));
        ...
        if (!empty($all_fields_details)) {
            ...
            if (!empty($post['AdminModulePeriodList'])) {
                $period_values = $post['AdminModulePeriodList'];
                $period_id = $period_values['id'];
                $period = $period_values['as_on'];
                ...
                $redirect_url = array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'edit', $model_id);
                $this->Session->write('Current.RedirectUrl', $redirect_url);
                $msg = array(
                    'type' => 'warning',
                    'title' => 'Warning... . . !',
            ...
            if (!empty($model_id)) {
                ...
                if (!empty($note_model_name)) {
                    ...
                    $note_conditions = array("$note_model_name.org_id" => $org_id, "$note_model_name.parent_model_id" => $model_id, "$note_model_name.model_id" => $note_model_id);

                    if (!empty($statement_year_period_field) && !empty($statement_year_period_id) && $this->$note_model_name->hasField($statement_year_period_field)) {
                        $note_conditions["$note_model_name.$statement_year_period_field"] = $statement_year_period_id;
                    }
                }
            }
            ...
            foreach ($all_fields_details as $field_details) {
                ...
                if ($has_notes && !empty($note_model_name) && !empty($note_conditions)) {
                    $note_conditions["$note_model_name.parent_field_id"] = $field_id;
                    $new_field_details['is_note_added'] = $this->$note_model_name->hasAny($note_conditions);
                }
...
//                                $options = Hash::combine($options_list, "{n}.$select_option_model.$dropdown_value_field", "{n}.$select_option_model.$dropdown_display_field", "{n}.$select_option_model.$dependent_dropdown_condition_field");
    ...
    public function save_note($parent_model_id = null, $note_model_id = null, $statement_year_period_id = null, $all_notes_data = null) {

        if (empty($all_notes_data) || empty($parent_model_id) || empty($note_model_id) || $statement_year_period_id == null)
            return;

        $this->loadModel('LookupModelDefinition');
        $note_model_name = $this->LookupModelDefinition->field('model_name', array('LookupModelDefinition.id' => $note_model_id));

        if (empty($note_model_name))
            return;

        ksort($all_notes_data, true);
        ...
        $note_data['parent_model_id'] = $parent_model_id;
        $note_data['model_id'] = $note_model_id;

        $conditions = array("$note_model_name.org_id" => $org_id, "$note_model_name.parent_model_id" => $parent_model_id, "$note_model_name.model_id" => $note_model_id);

        $this->loadModel($note_model_name);

        if (!empty($statement_year_period_id)) {
            if ($this->$note_model_name->hasField('statement_year_id')) {
                $note_data['statement_year_id'] = $statement_year_period_id;
                $conditions["$note_model_name.statement_year_id"] = $statement_year_period_id;
            } elseif ($this->$note_model_name->hasField('period_id')) {
                $note_data['period_id'] = $statement_year_period_id;
                $conditions["$note_model_name.period_id"] = $statement_year_period_id;
            }
        }

        $parent_field_ids = array();
        foreach ($all_notes_data as $parent_field_id => $note_list) {
            array_push($parent_field_ids, $parent_field_id);
            $note_data['parent_field_id'] = $parent_field_id;

            foreach ($note_list as $note_field_id => $note) {
                $note_data['field_id'] = $note_field_id;

                $note_data['note_values_int_type'] = $null_value;
                $note_data['note_values_double_type'] = $null_value;
                $note_data['note_values_varchar_type'] = $null_value;
                $note_data['note_values_date_type'] = $null_value;

                if (is_numeric($note)) {
                    if (ctype_digit(strval($note)))
                        $note_data['note_values_int_type'] = $note;
                    else
                        $note_data['note_values_double_type'] = $note;
                }
                elseif (is_string($note)) {
                    if (strtotime($note)) {
                        $note_data['note_values_date_type'] = $note;
                    } else {
                        $note_data['note_values_varchar_type'] = $note;
                    }
                }

                $notes_data = array_merge($notes_data, array($note_data));
            }
        }

        if (!empty($notes_data)) {
            try {
                $conditions["$note_model_name.parent_field_id"] = $parent_field_ids;
                $this->$note_model_name->recursive = -1;
                $this->$note_model_name->deleteAll($conditions, false);
            } catch (Exception $ex) {
                ////debug($ex);
            }
            $this->$note_model_name->create();
            $this->$note_model_name->saveAll($notes_data);
        }
    }

    public function add_note($parent_model_id = null, $parent_field_id = null, $note_model_id = null) {

        if (empty($parent_model_id) || empty($parent_field_id) || empty($note_model_id))
            return;
        ...
        $note_model_details = $this->LookupModelFieldDefinition->LookupModelDefinition->find('first', array('fields' => array('model_name', 'model_description'), 'conditions' => array('LookupModelDefinition.id' => $note_model_id), 'recursive' => -1));
        if (empty($note_model_details) || empty($note_model_details['LookupModelDefinition'])) {
            $msg = array(
                'type' => 'warning',
                'title' => 'Warning... . . !',
                'msg' => 'Invalid model information !'
        ...
        $note_model_name = $note_model_details['LookupModelDefinition']['model_name'];
        $title = $note_model_details['LookupModelDefinition']['model_description'];

        if ($this->request->is('post')) {
            if (!empty($this->request->data[$note_model_name])) {
                $notes_data = $this->Session->read('Notes.Data');
                $notes_data[$parent_field_id] = $this->request->data[$note_model_name];
                $this->Session->write('Notes.Data', $notes_data);
                return;
            }
        } else {
            $notes_data = $this->Session->read('Notes.Data');
            if (!$this->request->data && !empty($notes_data[$parent_field_id])) {
                $this->request->data = array($note_model_name => $notes_data[$parent_field_id]);
            }
        }
        ...
        if (!empty($period_values['AdminModulePeriodList'])) {
            ...
            $redirect_url = array('controller' => 'AdminModuleDynamicCrudFormGenerators', 'action' => 'add', $parent_model_id);
            $this->Session->write('Current.RedirectUrl', $redirect_url);
        ...
        $all_fields_details = $this->LookupModelFieldDefinition->find('all', array('conditions' => array('LookupModelFieldDefinition.model_id' => $note_model_id, array('or' => array('LookupModelFieldDefinition.parent_field_id' => $parent_field_id, 'LookupModelFieldDefinition.parent_field_id IS NULL')), 'LookupModelFieldDefinition.display_in_add_page' => 1), 'recursive' => -1, 'order' => array('LookupModelFieldDefinition.field_sorting_order' => 'asc')));
        if (!empty($all_fields_details)) {
            $new_field_details = array();
            $field_details_list = array();
            ...
            foreach ($all_fields_details as $field_details) {
                ...
                if ($control_type == "label") {
                    ...
                    $label_values = $this->LookupModelFieldDefinition->find('first', array('conditions' => array('LookupModelFieldDefinition.id' => $parent_field_id)));
                    $labels = $label_values[$associated_model_name][$associated_field_name_to_show];
                }
            ...
            $legend_groups = $this->LookupModelFieldDefinition->LookupModelFieldGroup->find('list', array('fields' => array('id', 'field_group_title'), 'conditions' => array('LookupModelFieldGroup.model_id' => $note_model_id)));
            $legend_sub_groups = $this->LookupModelFieldDefinition->LookupModelFieldSubGroup->find('list', array('fields' => array('id', 'field_sub_group_title', 'field_group_id'), 'conditions' => array('LookupModelFieldSubGroup.model_id' => $note_model_id)));

//            $sub_groups = $this->LookupModelFieldDefinition->LookupModelFieldSubGroup->find('all', array('fields' => array('id', 'field_group_id', 'field_sub_group_title'), 'conditions' => array('LookupModelFieldSubGroup.model_id' => $note_model_id)));
//            $legend_sub_groups = Hash::combine($sub_groups, '{n}.LookupModelFieldSubGroup.id', '{n}.LookupModelFieldSubGroup.field_sub_group_title', '{n}.LookupModelFieldSubGroup.field_group_id');
            ...
            $this->set(compact('title', 'note_model_id', 'note_model_name', 'parent_model_id', 'parent_field_id', 'legend_groups', 'legend_sub_groups', 'field_details_list'));
        } else {
            $msg = array(
                'type' => 'warning',
                'title' => 'Warning... . . !',
    ...
    public function edit_note($parent_model_id = null, $parent_field_id = null, $note_model_id = null, $statement_year_period_id = null, $is_editable = false) {

        if (empty($parent_model_id) || empty($parent_field_id) || empty($note_model_id) || empty($statement_year_period_id))
            return;
        ...
        if ($this->request->is(array('post', 'put'))) {
            if (!empty($this->request->data[$note_model_name])) {
                $all_notes_data = array($parent_field_id => $this->request->data[$note_model_name]);
                $this->save_note($parent_model_id, $note_model_id, $statement_year_period_id, $all_notes_data);
                return;
            }
        }
...
//            $legend_sub_groups = Hash::combine($sub_groups, '{n}.LookupModelFieldSubGroup.id', '{n}.LookupModelFieldSubGroup.field_sub_group_title', '{n}.LookupModelFieldSubGroup.field_group_id');
            ...
            if (!$this->request->data) {
                ...
                if ($statement_year_period_id != null) {
                    if ($this->$note_model_name->hasField('statement_year_id')) {
                        $note_data['statement_year_id'] = $statement_year_period_id;
                        $conditions["$note_model_name.statement_year_id"] = $statement_year_period_id;
                    } elseif ($this->$note_model_name->hasField('period_id')) {
                ...
                $this->$note_model_name->virtualFields = array('note_values' => "CONCAT_WS(' ', note_values_int_type, note_values_double_type, note_values_varchar_type, note_values_date_type)");
                $all_note_values = $this->$note_model_name->find('list', array('fields' => array('field_id', 'note_values'), 'conditions' => $conditions, 'recursive' => -1));

                $this->request->data = array($note_model_name => $all_note_values);
            }
        } else {
            $msg = array(
                'type' => 'warning',
