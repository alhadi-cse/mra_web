...
class LicenseModuleCancelByMraFieldInspectionDetailsController extends AppController {
    ...
    public function view($opt = 'all', $mode = null) {
        ...
        if (!empty($org_id)) {
            $condition = array_merge(array('LicenseModuleCancelByMraFieldInspectionDetail.org_id' => $org_id), $condition);
        }

        if ($this->request->is('post')) {
            $option = $this->request->data['LicenseModuleCancelByMraFieldInspectionDetail']['search_option'];
            $keyword = $this->request->data['LicenseModuleCancelByMraFieldInspectionDetail']['search_keyword'];
        ...
        $condition2 = array('licensing_year' => $current_year, 'licensing_state_id' => $thisStateIds[1], 'LicenseModuleCancelByMraFieldInspectionDetail.is_approved' => -1);
        $condition3 = array('licensing_year' => $current_year, 'licensing_state_id' => $thisStateIds[1], 'LicenseModuleCancelByMraFieldInspectionDetail.is_approved' => 0);
        $condition4 = array('licensing_year' => $current_year, 'licensing_state_id' => $thisStateIds[2], 'LicenseModuleCancelByMraFieldInspectionDetail.is_approved' => 1);
        ...
        if (in_array($inspector_group_id,$user_group_id)) {
            ...
            $condition2 = array_merge($condition2, array('LicenseModuleCancelByMraFieldInspectionDetail.org_id' => $this_user_org_id_list));
            $condition3 = array_merge($condition3, array('LicenseModuleCancelByMraFieldInspectionDetail.org_id' => $this_user_org_id_list));
            ...
            if (!empty($approved_org_id_list))
                $condition3 = array_merge($condition3, array('NOT' => array('LicenseModuleCancelByMraFieldInspectionDetail.org_id' => $approved_org_id_list)));
        }
        ...
        $this->LicenseModuleCancelByMraFieldInspectionDetail->recursive = 0;
        $values_not_inspect = $this->LicenseModuleCancelByMraFieldInspectionDetail->BasicModuleBasicInformation->find('all', array('fields' => $all_fields, 'conditions' => $condition1));

        $values_inspected_not_submit = $this->LicenseModuleCancelByMraFieldInspectionDetail->find('all', array('conditions' => $condition2, 'group' => array('org_id'), 'limit' => 10, 'order' => array('license_no' => 'asc')));

        $values_inspected_not_approved = $this->LicenseModuleCancelByMraFieldInspectionDetail->find('all', array('conditions' => $condition3, 'group' => array('org_id'), 'limit' => 10, 'order' => array('license_no' => 'asc')));

        $this->paginate = array('conditions' => $condition4, 'group' => array('org_id'), 'limit' => 10, 'order' => array('license_no' => 'asc'));
        $this->Paginator->settings = $this->paginate;
        $values_inspected = $this->Paginator->paginate('LicenseModuleCancelByMraFieldInspectionDetail');
    ...
    public function inspection($org_id = null) {
        ...
        if ($this->request->is('post')) {
            try {
                $newData = $this->request->data['LicenseModuleCancelByMraFieldInspectionDetail'];
                //debug($newData);
                //return;
                ...
                $this->LicenseModuleCancelByMraFieldInspectionDetail->create();
                if ($this->LicenseModuleCancelByMraFieldInspectionDetail->save($newData)) {
        ...
        $orgNameDetail = $this->LicenseModuleCancelByMraFieldInspectionDetail->BasicModuleBasicInformation->find('first', array('fields' => array('BasicModuleBasicInformation.full_name_of_org', 'BasicModuleBasicInformation.short_name_of_org'), 'conditions' => array('BasicModuleBasicInformation.id' => $org_id)));
    ...
    public function re_inspection($org_id = null, $option = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $this->LicenseModuleCancelByMraFieldInspectionDetail->id = $posted_data['id'];
            $done = $this->LicenseModuleCancelByMraFieldInspectionDetail->save($posted_data);
        ...
        $inspectionDetails = $this->LicenseModuleCancelByMraFieldInspectionDetail->find('first', array('conditions' => array('LicenseModuleCancelByMraFieldInspectionDetail.org_id' => $org_id, 'is_approved' => -1), 'order' => array('submission_date' => 'desc')));
    ...
    public function inspection_approval($org_id = null) {
                ...
                if ($option == 1) {
                    ...
                    if ($done) {
                        ...
                        if ((!empty($approve_count) && $approve_count == count($inspector_id_list))) {
                            ...
                            $this->LicenseModuleCancelByMraFieldInspectionDetail->updateAll(array('is_approved' => 1), array('org_id' => $org_id, 'is_approved' => 0));
                    ...
                    $done = $this->LicenseModuleCancelByMraFieldInspectionDetail->updateAll(array('is_approved' => -1), array('org_id' => $org_id, 'is_approved' => 0));
                        ...
                        if ($done) {
                            ...
                            if (!empty($inspector_email_list)) {
                                ...
                                try {
                                    ...
                                    $orgDetails = $this->LicenseModuleCancelByMraFieldInspectionDetail->BasicModuleBasicInformation->find('first', array('fields' => array('license_no', 'short_name_of_org', 'full_name_of_org'), 'conditions' => array('BasicModuleBasicInformation.id' => $org_id), 'recursive' => -1));
                                    if (!empty($orgDetails)) {
        ...
        $inspectionDetails = $this->LicenseModuleCancelByMraFieldInspectionDetail->find('first', array('conditions' => array('LicenseModuleCancelByMraFieldInspectionDetail.org_id' => $org_id, 'is_approved' => 0), 'order' => array('submission_date' => 'desc')));
    ...
    public function inspection_details($org_id = null) {
        ...
        if (!empty($org_id)) {
            ...
            $post = $this->LicenseModuleCancelByMraFieldInspectionDetail->find('first', array('conditions' => array('LicenseModuleCancelByMraFieldInspectionDetail.org_id' => $org_id), 'order' => array('submission_date' => 'desc')));
            if (!$this->request->data) {
    ...
    function inspector_select() {
        $this->layout = 'ajax';
        $org_id = $this->request->data['LicenseModuleCancelByMraFieldInspectionDetail']['org_id'];
        $this->loadModel('LicenseModuleCancelRequestFieldInspectorDetail');
        //$inspector_name_list = $this->LicenseModuleCancelRequestFieldInspectorDetail->find('list', array('fields' => 'name_with_designation_and_dept', 'recursive' => 0, 'conditions' => array('LicenseModuleCancelRequestFieldInspectorDetail.org_id' => $org_id)));
        $inspector_name_list = $this->LicenseModuleCancelRequestFieldInspectorDetail->find('list', array('fields' => 'name_with_designation_and_dept', 'recursive' => 0, 'conditions' => array('LicenseModuleCancelRequestFieldInspectorDetail.org_id' => $org_id, 'LicenseModuleCancelRequestFieldInspectorDetail.is_approved' => 1), 'group' => 'LicenseModuleCancelRequestFieldInspectorDetail.inspector_user_id'));
        $inspector_names = implode('<br />', $inspector_name_list);
