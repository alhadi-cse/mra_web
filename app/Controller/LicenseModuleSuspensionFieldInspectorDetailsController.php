...
class LicenseModuleSuspensionFieldInspectorDetailsController extends AppController {
    ...
    public function view() {        
        ...
        if (!empty($orgIds_assign)) {
            $allDistIds = $this->LicenseModuleSuspensionFieldInspectorDetail->find('list', array('fields' => 'district_id', 'conditions' => array('org_id' => $orgIds_assign), 'group' => array('org_id'))); //, 'conditions' => array('LicenseModuleSuspensionFieldInspectorDetail.district_id' => 'LookupAdminBoundaryDistrict.id')));

            if (!empty($allDistIds)) {

                $allDist = $this->LicenseModuleSuspensionFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => array('LookupAdminBoundaryDistrict.id', 'LookupAdminBoundaryDistrict.district_name'), 'conditions' => array('LookupAdminBoundaryDistrict.id' => $allDistIds))); //, 'conditions' => array('LicenseModuleSuspensionFieldInspectorDetail.district_id' => 'LookupAdminBoundaryDistrict.id')));
                foreach ($allDist as $dist_id => $dist_name) {

                    $allOrgIds = $this->LicenseModuleSuspensionFieldInspectorDetail->find('list', array('fields' => 'org_id', 'conditions' => array('district_id' => $dist_id), 'group' => 'org_id'));
                    if (empty($allOrgIds))
                        continue;
                    ...
                    foreach ($allOrgIds as $orgId) {
                        $allInspector = $this->LicenseModuleSuspensionFieldInspectorDetail->find('list', array('fields' => 'name_with_designation_and_dept', 'recursive' => 0, 'conditions' => array('LicenseModuleSuspensionFieldInspectorDetail.org_id' => $orgId)));
                        $condition_assign = array('BasicModuleBasicInformation.id' => $orgId,
                            'BasicModuleBasicInformation.licensing_year' => $current_year,
                            'BasicModuleBasicInformation.licensing_state_id' => $thisStateIds[2]);
                        
                        $fields = array('LicenseModuleSuspensionFieldInspectorDetail.inspection_date', 'BasicModuleBasicInformation.form_serial_no', 'BasicModuleBasicInformation.short_name_of_org', 'BasicModuleBasicInformation.full_name_of_org');
                        $orgDetails = $this->LicenseModuleSuspensionFieldInspectorDetail->find('first', array('fields' => $fields, 'recursive' => 0, 'conditions' => $condition_assign, 'group' => 'LicenseModuleSuspensionFieldInspectorDetail.org_id'));
                        ...
                        $ins_date = date("d-m-Y", strtotime($orgDetails['LicenseModuleSuspensionFieldInspectorDetail']['inspection_date']));
                        
                        ++$rowspan;
                        $row = $row . (empty($row) ? '' : '<tr>');
                        $row = $row . '<td style="text-align:center;">' . $orgDetails['BasicModuleBasicInformation']['form_serial_no'] . '</td>'
                                . '<td>' . $orgName . '</td>'
        ...
        $condition = array('BasicModuleBasicInformation.licensing_year' => $current_year, 'BasicModuleBasicInformation.licensing_state_id' => array($thisStateIds[0], $thisStateIds[1]), 'BasicModuleBranchInfo.office_type_id' => $office_type_id);
        $fields = array('BasicModuleBasicInformation.*', 'LookupAdminBoundaryDistrict.district_name');
       
        $this->loadModel('BasicModuleBranchInfo');
        $this->paginate = array('fields' => $fields, 'conditions' => $condition, 'limit' => 10, 'order' => array('form_serial_no' => 'asc'));
    ...
    public function assign() {
        ...
        $dist_list = array();
        $orgDetailsAll = null;
        //$condition = array('BasicModuleBasicInformation.licensing_year' => $current_year, 'BasicModuleBasicInformation.licensing_state_id' => $thisStateIds[0]);
        $condition = array('BasicModuleBasicInformation.licensing_year' => $current_year, 'BasicModuleBasicInformation.licensing_state_id' => array($thisStateIds[0], $thisStateIds[1]), 'BasicModuleBranchInfo.office_type_id' => $office_type_id);
        ...
        if (!empty($dist_ids)) {
            ...
            $dist_list = $this->LicenseModuleSuspensionFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => $fields, 'conditions' => $condition1));
...
//                    $dist_list = $this->LicenseModuleSuspensionFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => $fields, 'conditions' => $condition1));
//
//                    $fields = array('BasicModuleBasicInformation.id', 'BasicModuleBasicInformation.form_serial_no', 'BasicModuleBasicInformation.short_name_of_org', 'BasicModuleBasicInformation.full_name_of_org', 'BasicModuleBranchInfo.district_id');
//                    $orgDetailsAll = $this->BasicModuleBranchInfo->find('all', array('fields' => $fields, 'conditions' => $condition, 'order' => array('BasicModuleBranchInfo.district_id' => 'asc')));
//                }
//            }
//        }
        ...
        if ($this->request->is('post')) {
            ...
            $posted_data = $this->request->data['LicenseModuleSuspensionFieldInspectorDetail'];
            ...
            if (!empty($posted_data)) {
                ...
                if (!empty($new_data_to_save)) {
                    $this->LicenseModuleSuspensionFieldInspectorDetail->deleteAll(array('LicenseModuleSuspensionFieldInspectorDetail.org_id' => $all_org_ids), false);
                    
                    $this->LicenseModuleSuspensionFieldInspectorDetail->create();
                    $done = $this->LicenseModuleSuspensionFieldInspectorDetail->saveAll($new_data_to_save);
                    debug($done);
                    if ($done) {
                        $this->loadModel('BasicModuleBasicInformation');
    ...
    function organization_select_add() {
        ...
        $district_id = $this->request->data['LicenseModuleSuspensionFieldInspectorDetail']['district_id'];
        $this->loadModel('BasicModuleBranchInfo');
        ...
        foreach ($org_ids as $org_id) {
            ...
            $orgNames = $this->BasicModuleBasicInformation->find('all', array('fields' => array('BasicModuleBasicInformation.full_name_of_org', 'BasicModuleBasicInformation.form_serial_no'), 'conditions' => array('BasicModuleBasicInformation.id' => $orgId)));
            foreach ($orgNames as $value) {
                $orgName = $value['BasicModuleBasicInformation']['full_name_of_org'];
                $form_serial_no = $value['BasicModuleBasicInformation']['form_serial_no'];
            }
            $orgDetailsMerged = Hash::merge(array('org_id' => $orgId), array('org_name_serial' => $orgName . ' (Form Form No.: ' . $form_serial_no . ')'));
    ...
    function organization_select_edit() {
        ...
        if (!empty($district_id)) {
            ...
            $fields = array('BasicModuleBasicInformation.id', 'BasicModuleBasicInformation.form_serial_no', 'BasicModuleBasicInformation.short_name_of_org', 'BasicModuleBasicInformation.full_name_of_org');
            $condition = array('BasicModuleBasicInformation.licensing_year' => $current_year, 'BasicModuleBasicInformation.licensing_state_id' => array($thisStateIds[0], $thisStateIds[1]));
...
//                $passed_org_ids = $this->LicenseModuleInitialAssessmentDetail->find('list', array('fields' => array('LicenseModuleInitialAssessmentDetail.org_id'), 'recursive' => 0, 'conditions' => array_merge($condition, array('LicenseModuleInitialAssessmentMark.total_assessment_marks >= ' => $pass_min_marks)), 'group' => 'LicenseModuleInitialAssessmentDetail.org_id'));
                ...
                if (!empty($passed_org_ids)) {
                    ...
                    $this->loadModel('BasicModuleBranchInfo');
                    $this->BasicModuleBranchInfo->recursive = 0;
                    $organization_details = $this->BasicModuleBranchInfo->find('all', array('fields' => $fields, 'conditions' => $condition, 'order' => array('form_serial_no' => 'asc')));
                    $organization_list = Hash::combine($organization_details, '{n}.BasicModuleBasicInformation.id', array('<strong>%s</strong>: %s (Form No.: %s)', '{n}.BasicModuleBasicInformation.short_name_of_org', '{n}.BasicModuleBasicInformation.full_name_of_org', '{n}.BasicModuleBasicInformation.form_serial_no'));
                }
            }

            $selected_org_list = $this->LicenseModuleSuspensionFieldInspectorDetail->find('list', array('fields' => array('org_id'), 'conditions' => array('district_id' => $district_id), 'group' => array('org_id')));
        }
...
//            $inspector_list = $this->AdminModuleUserProfile->find('list', array('fields' => $fields, 'conditions' => $conditions));
            ...
            $selected_inspector_list = $this->LicenseModuleSuspensionFieldInspectorDetail->find('list', array('fields' => array('LicenseModuleSuspensionFieldInspectorDetail.inspector_user_id'), 'conditions' => array('LicenseModuleSuspensionFieldInspectorDetail.district_id' => $district_id)));
        }
    ...
    function grid_view_select() {
        $district_id = $this->request->data['LicenseModuleSuspensionFieldInspectorDetail']['district_id'];
        $condition = array('LicenseModuleSuspensionFieldInspectorDetail.district_id' => $district_id);
        $this->LicenseModuleSuspensionFieldInspectorDetail->recursive = 0;
        $this->Paginator->settings = $this->paginate;
        $values = $this->Paginator->paginate('LicenseModuleSuspensionFieldInspectorDetail');
        $this->paginate = array(
            'order' => array('LicenseModuleSuspensionFieldInspectorDetail.id' => 'ASC'),
            'limit' => 10,
            'conditions' => $condition);
        $field_inspection_values = $this->LicenseModuleSuspensionFieldInspectorDetail->find('all', array('conditions' => array('LicenseModuleSuspensionFieldInspectorDetail.district_id' => $district_id)));

        $this->loadModel('BasicModuleBasicInformation');
        $org_list_values = array();
        $inspector_list_values = array();
        foreach ($field_inspection_values as $value) {
            $org_id = $value['LicenseModuleSuspensionFieldInspectorDetail']['org_id'];
            $inspector_name = $value['LookupLicenseInspectorList']['inspector_name'];
            $org_names = $this->BasicModuleBasicInformation->find('all', array('fields' => array('BasicModuleBasicInformation.full_name_of_org', 'BasicModuleBasicInformation.form_serial_no'), 'conditions' => array('BasicModuleBasicInformation.id' => $org_id)));

            foreach ($org_names as $org_name) {
                $orgName = $org_name['BasicModuleBasicInformation']['full_name_of_org'];
                $form_serial_no = $org_name['BasicModuleBasicInformation']['form_serial_no'];
                $orgDetailsMerged = Hash::merge(array('org_id' => $org_id), array('org_name' => $orgName), array('form_serial_no' => $form_serial_no));
            }
    ...
    public function re_assign($id = null, $inspector_user_id = null) {
        ...
        $condition = array('BasicModuleBasicInformation.licensing_year' => $current_year,
        ...
        //$dist_list = $this->LicenseModuleSuspensionFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => array('LookupAdminBoundaryDistrict.id', 'LookupAdminBoundaryDistrict.district_name')));
        ...
        if ($this->request->is('post')) {
            try {
                ...
                $this->LicenseModuleSuspensionFieldInspectorDetail->deleteAll(array('LicenseModuleSuspensionFieldInspectorDetail.district_id' => $district_id), false);
                ...
                foreach ($org_ids as $org_id) {
                    foreach ($inspector_user_ids as $inspector_user_id) {
                        if ($inspector_user_id != "") {
                            ...
                            $this->LicenseModuleSuspensionFieldInspectorDetail->save($data_to_save);
                            $flag = true;
                        } else {
                            $flag = false;
                            break;
    ...
    public function deleteX($org_id) {
        $district_id = $this->request->data['LicenseModuleSuspensionFieldInspectorDetail']['district_id'];
        $inspector_group_id = $this->Session->read('Inspector.GroupId');        
        $this->LicenseModuleSuspensionFieldInspectorDetail->deleteAll(array('LicenseModuleSuspensionFieldInspectorDetail.district_id' => $district_id));
    ...
    public function delete($org_id) {
        ...
        if (!empty($org_id)) {
            if ($this->LicenseModuleSuspensionFieldInspectorDetail->deleteAll(array('LicenseModuleSuspensionFieldInspectorDetail.org_id' => $org_id), false)) {
                $current_year = $this->Session->read('Current.LicensingYear');
