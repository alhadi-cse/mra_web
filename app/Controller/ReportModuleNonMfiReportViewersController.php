...
//        return;
    ...
    public function report_viewer() {
        ...
        //model_ids=70^95&viewable_user_groups=1

        $model_ids = $this->request->query('model_ids');

        if (!empty($model_ids))
            $this->Session->write('Current.StateIds', $model_ids);
        else
            $model_ids = $this->Session->read('Current.StateIds');
...
//                if (count($modelIds) < 2) {
//                    $condition = array('id' => $modelIds);
//                } else {
//                    $condition = array('id >=' => $modelIds[0], 'id <=' => $modelIds[1]);
//                }
            }
        } else {
            $msg = array(
...
//        $field_list = $this->LookupModelFieldDefinition->find('list', array('fields' => array('field_id', 'field_title_for_report'), 'conditions' => $condition, 'recursive' => -1, 'group' => 'field_id', 'order' => 'field_id'));

        $report_title = 'Loan and Savings Reports';
        $this->set(compact('model_list', 'report_title'));
    }

    public function report_viewer_ok() {

        $this->loadModel('LookupModelFieldDefinition');
        $this->loadModel('ReportModuleReportFieldDefinition');
        //ReportModuleReportFieldDefinition

        $report_ids = null;
//        if ($this->request->is('post')) {
//            //debug($this->request->data['ReportModuleReportViewerSelect']['listReport']);
//            //$tbl_ids = $this->request->data['ReportModuleReportViewerReportSelect']['listReport'];
//            $report_ids = $this->request->data['ReportModuleReportViewerReportSelect']['listReport'];
//            
//            debug($this->request->data);
//
//
////            if (!empty($tbl_ids)) {
////                $condition = array('report_id' => $report_ids);
////                $report_fields = $this->ReportModuleReportFieldDefinition->find('list', array('fields' => array('ReportModuleReportFieldDefinition.field_id', 'CONCAT(ReportModuleReportFieldDefinition.field_title, " (", ReportModuleReportFieldDefinition.field_id, ")") field_title'), 'conditions' => $condition, 'recursive' => -1, 'group' => 'field_id', 'order' => 'field_id'));
////            } else if (!empty($report_ids)) {
////                $condition = array('report_id' => $report_ids);
////                $report_fields = $this->ReportModuleReportFieldDefinition->find('list', array('fields' => array('ReportModuleReportFieldDefinition.field_id', 'CONCAT(ReportModuleReportFieldDefinition.field_title, " (", ReportModuleReportFieldDefinition.field_id, ")") field_title'), 'conditions' => $condition, 'recursive' => -1, 'group' => 'field_id', 'order' => 'field_id'));
////            }
////
////            $this->set(compact('report_fields'));
//            //$this->render('report_viewer_selected_fields');
//            //return;
//        }
//            if ($this->request->is('post')) {
...
//            //$this->ReportModuleReportFieldDefinition->recursive = 0;
//            $this->Paginator->settings = $this->paginate;
//            $values = $this->Paginator->paginate('ReportModuleReportFieldDefinition');
//            $this->set(compact('values'));

        $condition = array('id >=' => 0);
        //$condition = array('id <' => 15);
//            $report_list = $this->ReportModuleReportFieldDefinition->ReportModuleReportDefinition->find('list', array('fields' => array('report_id', 'report_title'), 'conditions' => $condition, 'recursive' => 0, 'group' => 'report_title', 'order' => 'id'));
//
        $this->ReportModuleReportFieldDefinition->ReportModuleReportDefinition->virtualFields["report_title_with_id"] = "CONCAT(report_title, ' (', report_id, ')')";
        $report_list = $this->ReportModuleReportFieldDefinition->ReportModuleReportDefinition->find('list', array('fields' => array('report_id', 'report_title_with_id'), 'conditions' => $condition, 'recursive' => -1, 'order' => 'id'));
        //$report_list = $this->ReportModuleReportFieldDefinition->ReportModuleReportDefinition->find('list', array('fields' => array('ReportModuleReportDefinition.report_id', 'CONCAT(ReportModuleReportDefinition.report_title, " (", ReportModuleReportDefinition.report_id, ")")'), 'conditions' => $condition, 'recursive' => -1, 'order' => 'id'));
//            debug($report_list);
//                $condition = array('report_id' => $report_ids);
//                $field_list = $this->ReportModuleReportFieldDefinition->find('list', array('fields' => array('field_id', 'field_title'), 'conditions' => $condition, 'recursive' => -1, 'group' => 'field_id', 'order' => 'field_id'));
...
//        $field_list = $this->LookupModelFieldDefinition->find('list', array('fields' => array('field_id', 'field_title_for_report'), 'conditions' => $condition, 'recursive' => -1, 'group' => 'field_id', 'order' => 'field_id'));
        ...
        $this->set(compact('report_list', 'model_list'));
    }
...
//        return;
        ...
        if (!empty($this->request->data['ReportModuleReportViewerReportSelect'])) {
...
//                }
                ...
                $org_list = null;
//                try {
//                    $basic_model_name = ($model_id < 95) ? 'BasicModuleBasicInformation' : 'CDBNonMfiBasicInfo'; //CDBNonMfiBasicInfo
//                    if ($this->$model_name->hasField('org_id') && $this->$model_name->$basic_model_name->hasField('name_of_org', true)) {
//                        if ($model_id < 95 && $this->$model_name->$basic_model_name->isVirtualField('name_of_org')) {
//                            $this->$model_name->virtualFields['name_of_org'] = $this->$model_name->$basic_model_name->virtualFields['name_of_org'];
//                            $fields = array('org_id', 'name_of_org');
//                        } else {
//                            $fields = array('org_id', "$basic_model_name.name_of_org");
//                        }
//
//                        $condition = array("$basic_model_name.id >" => 0);
//                        $org_list = $this->$model_name->find('list', array('fields' => $fields, 'conditions' => $condition, 'recursive' => 0, 'group' => array('org_id')));
//                    }
//                } catch (Exception $ex) {
//                    debug($ex->getMessage());
//                }
        ...
        $this->set(compact('model_id', 'report_list', 'report_fields_list'));
    }
...
//                $model_details = $this->LookupModelFieldDefinition->LookupModelDefinition->findById($model_id);
//                debug($model_details);
...
//                    $asso_models_list = array_keys($asso_models_list);
                ...
                foreach ($field_details_list as $field_id => $field_details) {
                    ...
                    if (empty($field_details['LookupModelFieldDefinition'])) {
                        unset($field_details_list[$field_id]);
                        continue;
                    }
                    ...
                    if (strpos($field_model_name, '.') !== false) {
                        ...
                        if ($model_name == $field_parent_model_name) {
                            ...
                            if (!isset($asso_models_list[$field_model_name]) ||
                                    (!$this->$model_name->$field_model_name->hasField($field_name) &&
                                    !$this->$model_name->$field_model_name->isVirtualField($field_name))) {
                                unset($field_details_list[$field_id]);
                                continue;
                            }
                        ...
                        if ($model_name != $field_model_name) {
                            ...
                            if (!isset($asso_models_list[$field_model_name]) ||
                                    (!$this->$model_name->$field_parent_model_name->$field_model_name->hasField($field_name) &&
                                    !$this->$model_name->$field_parent_model_name->$field_model_name->isVirtualField($field_name))) {
                                unset($field_details_list[$field_id]);
                                continue;
                            }
...
//                debug($report_query_options);
                ...
                $all_data = array();
                try {
                    $all_data = $this->$model_name->find('all', $report_query_options); //, $model_name.branch_id
//                debug($all_data);
                ...
                $model_wise_data_details[$model_name] = array('all_data' => $all_data, 'selected_field_list' => $selected_field_list, 'fixed_field_details' => $fixed_field_details,
                    'field_group_list' => $field_group_list, 'group_wise_field_title' => $group_wise_field_title, 'group_wise_field_sub_title' => $group_wise_field_sub_title);
            } catch (Exception $ex) {
                debug($ex);
            }
        }
...
//        $unbind_models = array('belongsTo' => array('LookupBasicOfficeType', 'LookupAdminBoundaryDistrict', 'LookupAdminBoundaryUpazila', 'LookupAdminBoundaryUnion', 'LookupAdminBoundaryMauza'));
//        $this->loadModel("BasicModuleBranchInfo");
        ...
        foreach ($branchInfos as $branchInfo) {
            try {
                $branch_infos[] = array(
                    ...
                    'image_name' => $branchInfo["BasicModuleBranchInfo"]["image_name"],
    ...
    function branch_info() {
        ...
        foreach ($branchInfos as $branchInfo) {
            $branch_infos[] = array(
                ...
                'image_name' => $branchInfo["image_name"]
            );
        }
