...
//                $model_description = $this->LookupModelFieldDefinition->LookupModelDefinition->field('model_description', array('id' => $model_id));
                ...
                $model_name = $model_details['LookupModelDefinition']['model_name'];
                $report_title = $model_details['LookupModelDefinition']['model_description'];
...
//                $report_title = $model_description;
...
//                debug($all_data);
                ...
                $is_sub_title = true;
                if (!empty($all_data) && count($all_data) > 0 && !empty($selected_field_list)) {
                    $dr_header_group = '';
                    $dr_header_title = '';
                    $dr_header_sub_title = '';
                    $dr_all_data = '';
                    ...
                    $rc = 0;
                    $main_data_table = $fixed_data_table = $total_records_count = '';
                    ...
                    if (!empty($fixed_field_details['fixed_field_list']) && count($fixed_field_details['fixed_field_list']) > 0) {
                        ...
                        foreach ($fixed_field_list as $field_id => $fixed_field) {
                            foreach ($group_wise_field_title as $group_id => $field_list) {
                                ...
                                $fixed_table_tr .= (($rc % 2 == 0) ? "<tr>" : "<tr class='odd'>")
                                        . "<td style='min-width:170px; font-weight:bold;'>$fixed_data_title</td>"
                                        //. "<td style='font-weight:bold;'>:</td>"
                                        . "<td style='width:90%; text-align:left;'>$fixed_data_value</td></tr>";
                                ++$rc;
                        ...
                        if (!empty($fixed_table_tr))
                            $fixed_data_table = "<table class='tbl-rpt-view' style='width:100%; margin:0; padding:0;'>" . $fixed_table_tr . "</table>";
                    }
                    ...
                    if (!empty($dr_all_data)) {
                        $main_data_table = "<table id='tbl_data' class='tablesorter' style='width:100%; margin:0; padding:0;'>"
                                . "<thead>"
                                . (!empty($dr_header_group) ? "<tr>$dr_header_group</tr>" : "")
                                . (!empty($dr_header_title) ? "<tr>$dr_header_title</tr>" : "")
                                . (($is_sub_title && !empty($dr_header_sub_title)) ? "<tr>$dr_header_sub_title</tr>" : "")
                                . "</thead> <tbody>"
                                . $dr_all_data
                                . "</tbody>"
                                . $dr_total_value
                        ...
                        $total_records_count = count($all_data);
                    }

                    $this->set(compact('report_title', 'main_data_table', 'fixed_data_table', 'total_records_count'));
                } else {
                    $msg = array(
                        'type' => 'error',
                        'title' => 'Error... ... !',
...
//        $org_list = $this->BasicModuleBranchInfo->BasicModuleBasicInformation->find('list', array('fields' => array('id', 'name_of_org'), 'recursive' => -1, 'group' => array('BasicModuleBranchInfo.org_id'), 'order' => 'name_of_org'));
        ...
        $this->loadModel('AdminModulePeriodList');
        $period_list = $this->AdminModulePeriodList->find('list', array('fields' => array('AdminModulePeriodList.id', 'AdminModulePeriodList.period'), 'conditions' => array('AdminModulePeriodList.type_id' => 2), 'recursive' => 0, 'order' => 'AdminModulePeriodList.id'));

        $this->set(compact('org_id', 'org_list', 'period_list', 'admin_code', 'branch_infos'));
    }

    function get_branche_count_info_OK($admin_code = null, $org_id = null, $branch_type_id = null) {
    ...
    function get_loan_info($admin_code = null, $model_name = null, $field_name = null, $org_id = null, $period_id = null, $cat_id = null, $cat_sub_id = null, $cat_sub_sub_id = null) {
        ...
        try {
            ...
            if ($admin_code == "dist") {
                ...
                $fields = array("BasicModuleBranchInfo.district_id AS __geo_code",
                    "LookupAdminBoundaryDistrict.district_name AS __geo_name",
                    "SUM($model_name.$field_name) AS __data_value");
                $group_field_list = array("BasicModuleBranchInfo.district_id");
                $order_field_list = array("LookupAdminBoundaryDistrict.id ASC");
            } else if ($admin_code == "upaz") {
                $joins = array(
...
//                $org_id = $org_id = $this->Session->read('Org.Id');
        ...
        if (!empty($period_id))
            $conditions = array("$model_name.period_id" => $period_id);
        if (!empty($cat_id))
            $conditions = array("$model_name.loan_category_id" => $cat_id);
        if (!empty($cat_sub_id))
            $conditions = array("$model_name.loan_sub_category_id" => $cat_sub_id);
        if (!empty($cat_sub_sub_id))
            $conditions = array("$model_name.loan_sub_sub_category_id" => $cat_sub_sub_id);
...
//        $this->$model_name->BasicModuleBranchInfo->unbindModel($unbind_models, true);
        ...
        //$loanInfos = $this->$model_name->find("all", array('fields' => $fields, 'limit' => 2, 'recursive' => 0));
        //debug($loanInfos);
    ...
    function get_branches_location_ok($org_id = null, $branch_type_id = null) {
...
//            debug($branchInfos);
    ...
    //, $branch_type_id = 1
    function branches_location_map_ok($org_id = null, $branch_type_id = 1) {
...
//RMO
    function get_all_info($admin_code = null, $org_id = null, $model_name = null, $field_name = null, $period_id = null, $branch_type_id = null, $cat_id = null, $cat_sub_id = null, $cat_sub_sub_id = null) {
        ...
        if ($model_name == 2) {
            $branch_type_id = 3;
            $model_name = 'LoanModuleBranchwiseLoanInformation';
        } else if ($model_name == 3) {
            $branch_type_id = 3;
            $model_name = 'SavingsModuleBranchwiseSavingsInformation';
        }

        $model_name = empty($model_name) ? 'LoanModuleBranchwiseLoanInformation' : $model_name;

        if (empty($field_name)) {
            $field_name = 'no_of_borrowers';
        } else {
            $field_name = urlencode($field_name);
        }
        
//        else if ($field_name == "total") {
//            $field_name = "number_of_male_savers + number_of_female_savers";
//        }

        $fields = $joins = $group_field_list = $order_field_list = array();
        ...
        try {
            if ($this->$model_name->hasField('branch_id')) {
                $joins[] = array(
                    'table' => 'basic_module_branch_infos',
                    'alias' => 'BasicModuleBranchInfo',
                    'type' => 'LEFT',
                    'conditions' => array("BasicModuleBranchInfo.id = $model_name.branch_id"));
            }

            if ($admin_code == "dist" && $this->$model_name->BasicModuleBranchInfo->hasField('district_id')) {
                $joins[] = array(
                    'table' => 'lookup_admin_boundary_districts',
                    'alias' => 'LookupAdminBoundaryDistrict',
                    'type' => 'LEFT',
                ...
                $group_field_list = array("BasicModuleBranchInfo.district_id");
                $order_field_list = array("LookupAdminBoundaryDistrict.id ASC");
            } else if ($admin_code == "upaz" && $this->$model_name->BasicModuleBranchInfo->hasField('upazila_id')) {
                $joins[] = array(
                    'table' => 'lookup_admin_boundary_upazilas',
                    'alias' => 'LookupAdminBoundaryUpazila',
                    'type' => 'LEFT',
...
//            debug($ex->getMessage());
        ...
        if (!empty($org_id)) {
            $conditions["$model_name.org_id"] = $org_id;
        }
//        if (!empty($org_id))
//            $conditions = array("$model_name.org_id" => $org_id);
        ...
        if (!empty($period_id)) {
            $conditions["$model_name.period_id"] = $period_id;
        }
//        if (!empty($cat_id)){
//            $conditions["$model_name.loan_category_id"]= $cat_id;
//        }
//        if (!empty($cat_sub_id)){
//            $conditions["$model_name.loan_sub_category_id"]=$cat_sub_id;
//        }
//        if (!empty($cat_sub_sub_id)){
//            $conditions["$model_name.loan_sub_sub_category_id"] = $cat_sub_sub_id;
//        }
//        $unbind_models = array('belongsTo' => array('BasicModuleBasicInformation', 'LookupBasicOfficeType', 'LookupAdminBoundaryDistrict', 'LookupAdminBoundaryUpazila', 'LookupAdminBoundaryUnion', 'LookupAdminBoundaryMauza'));
//        $this->$model_name->BasicModuleBranchInfo->unbindModel($unbind_models, true);

        $allInfos = $this->$model_name->find("all", array('fields' => $fields, 'conditions' => $conditions, 'joins' => $joins, 'recursive' => -1, 'group' => $group_field_list, 'order' => $order_field_list));

        $all_infos = array();
        foreach ($allInfos as $dataRow) {
            if (!isset($dataRow[0]["__geo_code"]) || !isset($dataRow[0]["__geo_name"])) {
                continue;
            }

            $all_infos[] = array(
                'geo_code' => $dataRow[0]["__geo_code"],
                'geo_name' => $dataRow[0]["__geo_name"],
                'data_value' => empty($dataRow[0]["__data_value"]) ? 0 : $dataRow[0]["__data_value"]
            );
        }
    ...
    function get_branches_location($org_id = null, $branch_type_id = null, $dist_code = null, $upz_code = null) {      //RMO
        $conditions = array();
        ...
        if (!empty($dist_code)) {
            $conditions['BasicModuleBranchInfo.district_id'] = $dist_code;
        }
        if (!empty($upz_code)) {
            $conditions['BasicModuleBranchInfo.upazila_id'] = $upz_code;
        }

        //$conditions['NOT'] = array('BasicModuleBranchInfo.upazila_id' => $upz_code);
        ...
        $branchInfos = $this->BasicModuleBranchInfo->find("all", array('fields' => $fields, 'conditions' => $conditions, 'recursive' => 0));
    ...
    function get_branches_locationP($org_id = null, $branch_type_id = null, $dist_code = null, $upz_code = null) {      //RMO
//            if (empty($org_id))
//                $org_id = $org_id = $this->Session->read('Org.Id');
//       echo $branch_type_id;
//        //debug($branch_type_id);
//        exit();
        $conditions = array();
        if (!empty($org_id)) {
            $conditions['BasicModuleBranchInfo.org_id'] = $org_id;
...
//            debug($branchInfos);
    ...
    function branches_location_mapP($org_id = null, $branch_type_id = 1) {
        debug($org_id);
        if (empty($org_id))
            $org_id = $org_id = $this->Session->read('Org.Id');
