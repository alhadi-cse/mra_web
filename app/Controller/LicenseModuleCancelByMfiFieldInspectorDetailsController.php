...
class LicenseModuleCancelByMfiFieldInspectorDetailsController extends AppController {
    ...
    public function view() {        
        ...
        if (!empty($orgIds_assign)) {
            $allDistIds = $this->LicenseModuleCancelByMfiFieldInspectorDetail->find('list', array('fields' => 'district_id', 'conditions' => array('org_id' => $orgIds_assign), 'group' => array('org_id'))); //, 'conditions' => array('LicenseModuleCancelByMfiFieldInspectorDetail.district_id' => 'LookupAdminBoundaryDistrict.id')));

            if (!empty($allDistIds)) {

                $allDist = $this->LicenseModuleCancelByMfiFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => array('LookupAdminBoundaryDistrict.id', 'LookupAdminBoundaryDistrict.district_name'), 'conditions' => array('LookupAdminBoundaryDistrict.id' => $allDistIds))); //, 'conditions' => array('LicenseModuleCancelByMfiFieldInspectorDetail.district_id' => 'LookupAdminBoundaryDistrict.id')));
                foreach ($allDist as $dist_id => $dist_name) {

                    $allOrgIds = $this->LicenseModuleCancelByMfiFieldInspectorDetail->find('list', array('fields' => 'org_id', 'conditions' => array('district_id' => $dist_id), 'group' => 'org_id'));
                    if (empty($allOrgIds))
                        continue;
                    ...
                    foreach ($allOrgIds as $orgId) {
                        $allInspector = $this->LicenseModuleCancelByMfiFieldInspectorDetail->find('list', array('fields' => 'name_with_designation_and_dept', 'recursive' => 0, 'conditions' => array('LicenseModuleCancelByMfiFieldInspectorDetail.org_id' => $orgId)));
                        $condition_assign = array('BasicModuleBasicInformation.id' => $orgId,
                            
                            'BasicModuleBasicInformation.licensing_state_id' => $thisStateIds[2]);
                        $fields = array('LicenseModuleCancelByMfiFieldInspectorDetail.inspection_date', 'BasicModuleBasicInformation.license_no', 'BasicModuleBasicInformation.short_name_of_org', 'BasicModuleBasicInformation.full_name_of_org');
                        $orgDetails = $this->LicenseModuleCancelByMfiFieldInspectorDetail->find('first', array('fields' => $fields, 'recursive' => 0, 'conditions' => $condition_assign, 'group' => 'LicenseModuleCancelByMfiFieldInspectorDetail.org_id'));
                        ...
                        $ins_date = date("d-m-Y", strtotime($orgDetails['LicenseModuleCancelByMfiFieldInspectorDetail']['inspection_date']));
    ...
    public function assign() {
        ...
        if (!empty($dist_ids)) {
            ...
            $dist_list = $this->LicenseModuleCancelByMfiFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => $fields, 'conditions' => $condition1));
        ...
        if ($this->request->is('post')) {
            ...
            $posted_data = $this->request->data['LicenseModuleCancelByMfiFieldInspectorDetail'];
            ...
            if (!empty($posted_data)) {
                ...
                if (!empty($new_data_to_save)) {
                    $this->LicenseModuleCancelByMfiFieldInspectorDetail->deleteAll(array('LicenseModuleCancelByMfiFieldInspectorDetail.org_id' => $all_org_ids), false);

                    $this->LicenseModuleCancelByMfiFieldInspectorDetail->create();
                    $done = $this->LicenseModuleCancelByMfiFieldInspectorDetail->saveAll($new_data_to_save);
                    debug($done);
                    if ($done) {
                        $this->loadModel('BasicModuleBasicInformation');
    ...
    function organization_select_add() {
        ...
        $district_id = $this->request->data['LicenseModuleCancelByMfiFieldInspectorDetail']['district_id'];
        $this->loadModel('BasicModuleBranchInfo');
...
//                $passed_org_ids = $this->LicenseModuleInitialAssessmentDetail->find('list', array('fields' => array('LicenseModuleInitialAssessmentDetail.org_id'), 'recursive' => 0, 'conditions' => array_merge($condition, array('LicenseModuleInitialAssessmentMark.total_assessment_marks >= ' => $pass_min_marks)), 'group' => 'LicenseModuleInitialAssessmentDetail.org_id'));
            ...
            $selected_org_list = $this->LicenseModuleCancelByMfiFieldInspectorDetail->find('list', array('fields' => array('org_id'), 'conditions' => array('district_id' => $district_id), 'group' => array('org_id')));
        }
...
//            $inspector_list = $this->AdminModuleUserProfile->find('list', array('fields' => $fields, 'conditions' => $conditions));
            ...
            $selected_inspector_list = $this->LicenseModuleCancelByMfiFieldInspectorDetail->find('list', array('fields' => array('LicenseModuleCancelByMfiFieldInspectorDetail.inspector_user_id'), 'conditions' => array('LicenseModuleCancelByMfiFieldInspectorDetail.district_id' => $district_id)));
        }
    ...
    function grid_view_select() {
        $district_id = $this->request->data['LicenseModuleCancelByMfiFieldInspectorDetail']['district_id'];
        $condition = array('LicenseModuleCancelByMfiFieldInspectorDetail.district_id' => $district_id);
        $this->LicenseModuleCancelByMfiFieldInspectorDetail->recursive = 0;
        $this->Paginator->settings = $this->paginate;
        $values = $this->Paginator->paginate('LicenseModuleCancelByMfiFieldInspectorDetail');
        $this->paginate = array(
            'order' => array('LicenseModuleCancelByMfiFieldInspectorDetail.id' => 'ASC'),
            'limit' => 10,
            'conditions' => $condition);
        $field_inspection_values = $this->LicenseModuleCancelByMfiFieldInspectorDetail->find('all', array('conditions' => array('LicenseModuleCancelByMfiFieldInspectorDetail.district_id' => $district_id)));

        $this->loadModel('BasicModuleBasicInformation');
        $org_list_values = array();
        $inspector_list_values = array();
        foreach ($field_inspection_values as $value) {
            $org_id = $value['LicenseModuleCancelByMfiFieldInspectorDetail']['org_id'];
    ...
    public function re_assign($id = null, $inspector_user_id = null) {
        ...
        //$dist_list = $this->LicenseModuleCancelByMfiFieldInspectorDetail->LookupAdminBoundaryDistrict->find('list', array('fields' => array('LookupAdminBoundaryDistrict.id', 'LookupAdminBoundaryDistrict.district_name')));
        ...
        if ($this->request->is('post')) {
            try {
                ...
                $this->LicenseModuleCancelByMfiFieldInspectorDetail->deleteAll(array('LicenseModuleCancelByMfiFieldInspectorDetail.district_id' => $district_id), false);
                ...
                foreach ($org_ids as $org_id) {
                    foreach ($inspector_user_ids as $inspector_user_id) {
                        if ($inspector_user_id != "") {
                            ...
                            $this->LicenseModuleCancelByMfiFieldInspectorDetail->save($data_to_save);
                            $flag = true;
                        } else {
                            $flag = false;
                            break;
    ...
    public function deleteX($org_id) {
        $district_id = $this->request->data['LicenseModuleCancelByMfiFieldInspectorDetail']['district_id'];
        $inspector_group_id = $this->Session->read('Inspector.GroupId');        
        $this->LicenseModuleCancelByMfiFieldInspectorDetail->deleteAll(array('LicenseModuleCancelByMfiFieldInspectorDetail.district_id' => $district_id));
    ...
    public function delete($org_id) {
        ...
        if (!empty($org_id)) {
            if ($this->LicenseModuleCancelByMfiFieldInspectorDetail->deleteAll(array('LicenseModuleCancelByMfiFieldInspectorDetail.org_id' => $org_id), false)) {
                $current_year = $this->Session->read('Current.LicensingYear');
