...
class LicenseModuleDirectSuspensionApplicationOfReviewDetailsController extends AppController {
    public $helpers = array('Form', 'Html', 'Js', 'Paginator', 'Time');
    public $components = array('Paginator');
    public $paginate = array();
    var $uses = array('BasicModuleBasicInformation','LicenseModuleDirectSuspensionApplicationOfReviewDetail');
    ...
    public function view($opt = 'all') {
        ...
        if ($this->request->is('post')) {
            if (!empty($this->request->data['LicenseModuleDirectSuspensionApplicationOfReviewDetailCompleted'])) {
                $reqData = $this->request->data['LicenseModuleDirectSuspensionApplicationOfReviewDetailCompleted'];
                $option = $reqData['search_option_completed'];
                $keyword = $reqData['search_keyword_completed'];
            } 
            elseif (!empty($this->request->data['LicenseModuleDirectSuspensionApplicationOfReviewDetailPending'])) {
                $reqData = $this->request->data['LicenseModuleDirectSuspensionApplicationOfReviewDetailPending'];
        ...
        $this->LicenseModuleDirectSuspensionApplicationOfReviewDetail->recursive = 0;
        $completed_values = $this->Paginator->paginate('LicenseModuleDirectSuspensionApplicationOfReviewDetail');       
    ...
    public function details($org_id = null) {
        ...
        $allDetails = $this->LicenseModuleDirectSuspensionApplicationOfReviewDetail->find('first', array('conditions' => array('org_id'=>$org_id)));
    ...
    public function apply($org_id = null) {
        ...
        if($this->request->is('post')){
            $review_application_details = $this->request->data['LicenseModuleDirectSuspensionApplicationOfReviewDetail']['review_application_details'];
            $data_to_save = array(
                'org_id' =>$org_id,
            ...
            $existing_value_condition = array('LicenseModuleDirectSuspensionApplicationOfReviewDetail.org_id'=>$org_id);
            $existing_values = $this->LicenseModuleDirectSuspensionApplicationOfReviewDetail->find('first',array('conditions'=>$existing_value_condition));
            if(!empty($existing_values)){                
                $this->LicenseModuleDirectSuspensionApplicationOfReviewDetail->deleteAll($existing_value_condition, false);
            }
            $this->LicenseModuleDirectSuspensionApplicationOfReviewDetail->create();
            $saved = $this->LicenseModuleDirectSuspensionApplicationOfReviewDetail->save($data_to_save);
