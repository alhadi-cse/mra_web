...
class LicenseModuleLicenseEvaluationAdminApprovalDetailsController extends AppController {
    ...
    public function view($opt = 'all', $mode = null) {
        ...
        $redirect_url = array('controller' => 'LicenseModuleLicenseEvaluationAdminApprovalDetails', 'action' => 'view', '?' => array('this_state_id' => $this_state_ids));
        ...
        if (!empty($org_id)) {
            $condition = array_merge(array('LicenseModuleLicenseEvaluationAdminApprovalDetail.org_id' => $org_id), $condition);
        }

        if ($this->request->is('post')) {
            $option = $this->request->data['LicenseModuleLicenseEvaluationAdminApprovalDetail']['search_option'];
            $keyword = $this->request->data['LicenseModuleLicenseEvaluationAdminApprovalDetail']['search_keyword'];
        ...
        $this->paginate = array('conditions' => $condition2, 'group' => array('LicenseModuleLicenseEvaluationAdminApprovalDetail.org_id'), 'limit' => 10, 'order' => array('form_serial_no' => 'asc'));
        $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->recursive = 0;
        $this->Paginator->settings = $this->paginate;
        $values_approved = $this->Paginator->paginate('LicenseModuleLicenseEvaluationAdminApprovalDetail');

        $this->set(compact('IsValidUser', 'org_id', 'user_group_id', 'opt_all', 'total_marks', 'pass_min_marks', 'values_approved', 'values_not_approved'));
        
        
//        
//        $values = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->find('all');//,array('group' => array('LicenseModuleLicenseEvaluationAdminApprovalDetail.org_id')));
//        debug($values);
    }
    ...
    public function approve_details($org_id = null) {
        ...
        $licApprovalDetails = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->find('first', array('conditions' => array('LicenseModuleLicenseEvaluationAdminApprovalDetail.org_id' => $org_id)));
    ...
    public function approve_all() {        
        ...
        if ($this->request->is('post')) {
            try {
                $newData = $this->request->data['LicenseModuleLicenseEvaluationAdminApprovalDetailAll'];
                if (!empty($newData)) {
        ...
        $approval_status_options = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->LookupLicenseApprovalStatus->find('list', array('fields' => array('LookupLicenseApprovalStatus.id', 'LookupLicenseApprovalStatus.approval_status')));
...
//                    //$newData = $this->request->data['LicenseModuleLicenseEvaluationAdminApprovalDetail'];
//                    if (!empty($nData)) {
//                        $id = $nData['id'];
//                        if (!empty($id)) {
//                            $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->id = $id;
//                            $nData = Hash::remove($nData, 'id');
//                            $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->save($nData);
//                        }
//                    }
//                }

                if (!empty($newData)) {
                    $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->set($newData);
                    if ($this->LicenseModuleLicenseEvaluationAdminApprovalDetail->saveAll($newData)) {
...
//        
//        $this->Paginator->settings = $this->paginate;
//        $orgDetails = $this->Paginator->paginate('LicenseModuleLicenseEvaluationAdminApprovalDetail');
    ...
    public function approve($org_id = null) {
        ...
        if ($this->request->is('post')) {
            try {
                ...
                if (!empty($newData)) {
                    $existingData = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->find('first', array('fields' => array('LicenseModuleLicenseEvaluationAdminApprovalDetail.id'), 'recursive' => 0, 'conditions' => array('LicenseModuleLicenseEvaluationAdminApprovalDetail.org_id' => $newData['org_id'])));
                    if ($existingData) {
                        $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->id = $existingData['LicenseModuleLicenseEvaluationAdminApprovalDetail']['id'];
                        $done = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->save($newData);
                    } else {
                        $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->create();
                        $done = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->save($newData);
                    }
        ...
        $orgFullName = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->BasicModuleBasicInformation->find('first', array('fields' => array('BasicModuleBasicInformation.full_name_of_org'), 'conditions' => array('BasicModuleBasicInformation.id' => $org_id)));
    ...
    public function re_approve($org_id = null) {
        ...
        $approvalDetails = $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->find('first', array('conditions' => array('LicenseModuleLicenseEvaluationAdminApprovalDetail.org_id' => $org_id)));
        ...
        if ($this->request->is(array('post', 'put'))) {
            $newData = $this->request->data['LicenseModuleLicenseEvaluationAdminApprovalDetail'];
            if (!empty($newData)) {
                $this->LicenseModuleLicenseEvaluationAdminApprovalDetail->id = $approvalDetails['LicenseModuleLicenseEvaluationAdminApprovalDetail']['id'];
                if ($this->LicenseModuleLicenseEvaluationAdminApprovalDetail->save($newData)) {
                    $this->redirect(array('action' => 'view'));
                }
            }
            return;
